...
$activeSheet->setCellValue(($first_column).($row++), $_DISTRICT ." District");
$activeSheet->setCellValue(($first_column).($row++), $_SECTOR ." Sector");
$activeSheet->setCellValue(($first_column).($row++), $organisation);
$activeSheet->setCellValue(($first_column).($row++), "Tel: ".$organisation_phone );
$activeSheet->setCellValue(($first_column).($row++), "Email: ".$organisation_email );
$activeSheet->setCellValue(($first_column).($row++), $report_title);

$valid_dates = array();
$tb_data = "";
$coartems = formatResultSet($rslt=returnResultSet($sql = "SELECT DISTINCT MedecineName FROM md_name WHERE md_name.MedecineName LIKE('%{$_GET['medicine']}%') || md_name.MedecineName LIKE('%quinine%') || md_name.MedecineName LIKE('%tesunate%') ORDER BY MedecineName",$con),$multirows=true,$con);
//echo $sql;
...
$row++;
$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
$activeSheet->setCellValue(($first_column++).($row), "Date");

$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
$activeSheet->setCellValue(($first_column++).($row), "No");

$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
$activeSheet->setCellValue(($first_column++).($row), "Patient Name");

$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
$activeSheet->setCellValue(($first_column++).($row), "Age");

$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
$activeSheet->setCellValue(($first_column++).($row), "Insurance ID");
...
$tb_data .= "</tr>";
SpanCells($activeSheet,"A1:F1",$align='left');
SpanCells($activeSheet,"A2:F2",$align='left');
SpanCells($activeSheet,"A3:F3",$align='left');

SpanCells($activeSheet,"A4:F4",$align='left');
SpanCells($activeSheet,"A5:F5",$align='left');

SpanCells($activeSheet,"A6:M6",$align='center');

SpanCells($activeSheet,"H7:K7",$align='center');
$activeSheet->setCellValue("H7", "Coartem");
// echo $row."<br />";
$row++;
$str .= "
<div style='max-height:490px; padding-top:5px; border:0px solid #000; overflow:auto;'>
<table id=new class=list border=1>
<tr><th colspan=6></th><th>&nbsp;</th><th colspan=4>Coartem</th><th colspan=3>&nbsp;</th></tr>
<tr><th>Date</th><th>No</th><th>Patient Name</th><th>Age</th><th>Insurance ID</th><th>Category</th>";
$str .= $tb_data;
...
$sql = "SELECT DISTINCT md_name.MedecineName, COALESCE(pa_records.InsuranceCardID, pa_records.applicationNumber, '') AS InsuranceCardID, pa_records.DateIn, pa_records.FamilyCategory, pa_info.*, md_records.Quantity FROM md_records, md_price, md_name, co_records, pa_records, in_name, pa_info, sy_users, sy_center
		WHERE 
...
for($i=0;$i<count($medecines);$i++){
	...
	if($medecines[$i]['Name'] != 'Total'){
		...
		$age = getAge($medecines[$i]['DateofBirth'],$notation=1, $current_date=$medecines[$i]['DateIn']);
		$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
		$activeSheet->setCellValue(($first_column++).($row), $age);
		...
		$str .= "<td colspan=6 style='text-align:center;'>&nbsp;</td>";
		
		SpanCells($activeSheet,($first_column.$row).":E".($row),$align='center');
...
$activeSheet->setTitle("Unpaid anti malaria");
