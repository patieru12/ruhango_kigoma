...
<table border=1><tr><th colspan=2>&nbsp;</th>";
$valid_dates = array();
...
$sql = "SELECT DISTINCT md_name.*, md_category.*, md_price.* from md_name, md_price, md_category WHERE md_name.MedecineCategorID = md_category.MedecineCategoryID && md_name.MedecineNameID = md_price.MedecineNameID  ORDER BY MedecineCategoryName ASC, MedecineName ASC";
...
for($i=0;$i<count($medecines);$i++){
	...
	$str .= "<tr>";
	$str .= "<td>".($count++)."</td>";
	$str .= "<td>{$medecines[$i]['MedecineName']} {$medecines[$i]['MedecinePriceID']}</td>";
	$tot = 0;
	foreach($valid_dates as $date){
		//select the sum of the current medecines for the current days
		$all_medecines = returnSingleField("SELECT SUM(Quantity) as SM FROM md_records WHERE MedecinePriceID='{$medecines[$i]['MedecinePriceID']}' && Date='{$date}'",$field="SM",$data=true, $con);
