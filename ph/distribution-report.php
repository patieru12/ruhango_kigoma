...
?>
  <div id="w" style='height: auto;'>
    <div id="content" style='height: auto;'>
    	<b>
		<h1 style='align: center; margin-top:-55px;'>Medicines Distribution Report</h1>
    	...
    	<div>
			...
			<div class=stock_status style='height:420px; width:100%; overflow:auto;'>
				<script>
					...
					$(".stock_status").load("./stock-distribution.php?start_date=" + $("#start_date").val() + "&end_date=" + $("#end_date").val());
				</script>
			</div>
		</div>
	...
	$(document).ready(function(){
		...
		$("#search").keyup(function(){
			...
			if(query_sent == false){
				...
				setTimeout(function(){
					if($("#search").val().trim != ""){
						$(".stock_status").load("./stock-distribution.php?start_date=" + $("#start_date").val() + "&end_date=" + $("#end_date").val() + "&filter=true&keyword=" + $("#search").val().replace(/ /g,"%20"));
						$("#bar").html("");
					}
					query_sent = false;
				},1500);
