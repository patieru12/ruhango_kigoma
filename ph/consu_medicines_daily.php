...
//echo (60*60*24);
//var_dump($_GET);
$original_time = $start_time = strtotime($_GET['startdate']); //mktime(0,0,0,$_GET['month'],1,$_GET['year']);
$end_date = strtotime($_GET['end_date']);
$step = (60*60*24);
$_GET['month'] = date("m", $start_time);
$_GET['year'] = date("Y", $start_time);
...
Post: {$post}<br />
Date: {$_GET['startdate']} to {$_GET['end_date']}
</span>
</span>
...
for($start_time; ($start_time <= $end_date); $start_time += $step){
...
for($i=0;$i<count($medecines);$i++){
	...
	foreach($valid_dates as $date){
		...
		$all_medecines = returnSingleField("SELECT COALESCE(SUM(Quantity),0) as SM FROM md_records WHERE {$cnd} && Date='{$date}'",$field="SM",$data=true, $con);
		if($consummables_)
			$all_medecines = returnSingleField("SELECT COALESCE(SUM(Quantity),0) as SM FROM cn_records WHERE {$cnd} && Date='{$date}'",$field="SM",$data=true, $con);
