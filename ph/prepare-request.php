				...
				<td>
					...
					if(@$_POST['save_date']){
						...
						if($_POST['medicine']){
							if($_POST['quantity']){
								...
								if(!$request_id = returnSingleField("SELECT StockRecordsID FROM md_stock_records WHERE MedicineStockID='{$stock_id}' && Operation='REQUEST' && Status=-1","StockRecordsID",true,$con))
									saveData("INSERT INTO md_stock_records SET MedicineStockID='{$stock_id}', Operation='REQUEST', Quantity='".PDB($_POST['quantity'],true,$con)."', Date='".time()."', UserID='".$_SESSION['user']['UserID']."', Status=-1",$con);
								else
					...
					if(@$_POST['confirm']){
						...
						foreach($_POST as $index=>$value){
							...
							if($st_id = returnAllData("SELECT * FROM md_stock_records WHERE StockRecordsID='".PDB($index,true,$con)."' && Status=-1",$con)[0]){
								
								//create a query to update the request
								$sql = "UPDATE md_stock_records SET Status=0 WHERE StockRecordsID='".PDB($index,true,$con)."'";
								//echo $sql."<br />";
								saveData($sql, $con);
...
/*
					...
					<form action="./prepare-request.php" method=post>
						<table class=list style='width:100%;'>
							<tr><th>#</th><th>Medicine</th><th>Quantity</th><th>Time</th><th colspan=2>Operation</th></tr>
							<?php
							if($request_data){
								$count = 1;
								foreach($request_data as $rqst){
									echo "<tr class='".($rqst["Status"] == 0?"rqstsent":"rqstnotsent")."'' title='".(!is_null($rqst["Comment"])?$rqst["Comment"]:"No Additional Info")."' >
									<td>".($rqst["Status"] == -1?"<input class=all type=checkbox checked ".(@$_POST[$rqst['StockRecordsID']]?"checked":"")." name='{$rqst['StockRecordsID']}' />":"&nbsp;").($count++)."</td>
									<td>{$rqst['MedecineName']}".(!is_null($rqst["Comment"])?" <i class='fa fa-comment-o text-success'></i>":"")."</td>
									<td>{$rqst['Quantity']}</td>
									<td>{$rqst['requestTime']}</td>
						...
						<label style='cursor:pointer; color:blue;'><input type='checkbox' checked="" id='control' onclick='if($("#control").prop("checked")){ $(".all").attr("checked",":true"); } else { $(".all").removeAttr("checked"); }' />check all</label><br />
						<div class="btn-group">
							<button type=submit id='confirm' title='Approve That you received Requested Medicines' class="btn btn-primary" style='font-size:10px;' name='confirm' value='Send Request'> <i class="fa fa-arrow-up"></i> Send Request</button>
							<!-- <button type=button name='print' onclick='click_now();' title='Print the list of requested medicines' class="btn btn-success" style='font-size:10px;'  value='Print Request'> <i class="fa fa-print"></i> Print Request</button> -->
						</div>
					</form>
