...
for($i=0;$i<count($medecines);$i++){
	...
	foreach($coartems as $date){
		...
		$all_medecines = preg_match("/oartem/",strtolower($medecines[$i]['MedecineName']))?($date['MedecineName'] == $medecines[$i]['MedecineName']?$medecines[$i]['Quantity']:""):($date['MedecineName'] == $medecines[$i]['MedecineName']?$medecines[$i]['Quantity']:"");//returnSingleField("SELECT SUM(Quantity) as SM FROM md_records WHERE {$cnd} && Date='{$date}'",$field="SM",$data=true, $con);
