...
$report_title = "Anti-Malaria Distribution";
...
$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
$activeSheet->setCellValue(($first_column++).($row), "Insurance");
...
SpanCells($activeSheet,"G4:j4",$align='center');
$activeSheet->setCellValue("G4", "Coartem");

$row = 6;
$str .= "
<div style='max-height:350px; padding-top:5px; border:0px solid #000; overflow:auto;'>
<table id=new class=list border=1>
<tr><th colspan=6></th><th colspan=4>Coartem</th><th colspan=3>&nbsp;</th></tr>
<tr><th>#</th><th>Date</th><th>Patient</th><th>Insurance</th><th>Code</th><th>Category</th>";
$str .= $tb_data;
...
$sql = "SELECT DISTINCT md_name.MedecineName, pa_records.InsuranceCardID, pa_records.DateIn, pa_records.FamilyCategory, pa_info.*, md_records.Quantity, in_name.InsuranceName FROM md_records, md_price, md_name, co_records, pa_records, in_name, pa_info, sy_users, sy_center
		WHERE 
...
for($i=0;$i<count($medecines);$i++){
	...
	if($medecines[$i]['Name'] != 'Total'){
		...
		$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
		$activeSheet->setCellValue(($first_column++).($row), $medecines[$i]['InsuranceName']);
		
		$str .= "<td>{$medecines[$i]['InsuranceName']}</td>";
		...
		$activeSheet->getColumnDimension($first_column)->setAutoSize(true);
		$activeSheet->setCellValue(($first_column++).($row), (strtolower($medecines[$i]['InsuranceName'])=="cbhi"?$medecines[$i]['FamilyCategory']:""));
		...
		SpanCells($activeSheet,($first_column.$row).":F".($row),$align='center');
...
$objPHPExcel->getActiveSheet()->getStyle("A4:M".(--$row))->applyFromArray($styleArray);
