...
if(strlen($_GET['key'])){
	...
	$sql = "SELECT 	a.*,
					...
					WHERE b.dailyID = '{$PatientID}' && b.DateIn = '{$dateRequest}'
	...
	} else if($patients[0]['InsuranceNameID'] != $InsuranceNameID){
		echo "<span class=error-text>The Patient is not registered by CBHI Agent</span> ".count($patients);
	}else if($patients){
		//echo count($patients);
		?>
		<script>
