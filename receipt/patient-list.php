...
<script type="text/javascript">
	function LoadProfile(patientID, printData="0"){
		// Load the Diagnostic Information from here
		$(".activePatient").removeClass("activeP");
		$("#myTr" + patientID).addClass("activeP");
		$("#receiptcontent").load("./receipt-content.php?patientID=" + patientID + "&print=" + printData);
...
if(@$_GET['records']){
	$printData = "\"0\"";
	if(@$_GET['print']){
		$printData = "\"".$_GET['print']."\"";
	}
	$data .= <<<AUTOCLICK
	<script type="text/javascript">
		setTimeout(function(){
			LoadProfile({$_GET['records']}, {$printData});
		}, 100);
	</script>
AUTOCLICK;
}
