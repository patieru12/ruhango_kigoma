...
for($k = 0; $k<count($_exams_); $k++){
	//var_dump($_exams_[$k]['ExamNumber']);
	$_exams_[$k]['ExamNumber'] = preg_match("/^".substr($_exams_[$k]['ResultDate'],0,4)."Q/",$_exams_[$k]['ExamNumber'])?substr($_exams_[$k]['ExamNumber'],6):$_exams_[$k]['ExamNumber'];
	//var_dump($_exams_[$k]['ExamNumber']);
...
<form action="./save_from_cons_doc.php" method=post style='border:1px solid #efefef;' id="frm_data">
	...
	<div style='height:1px; border:0px solid #000;'>
		<input type=text class=txtfield1 readonly style='width:75px; font-size:12px; position:relative; top: 18px; left:12px;' name=consultation_date id=consultation_date onclick='if(true){ return false; }ds_sh(this,"consultation_date")' value='<?= $patient['DateIn']?$patient['DateIn']:date("Y-m-d",time()) ?>' />
	</div>
	<table class=frm>
		<tr><td>Consultation</td>
		<th>
			<input type=hidden id=cons_date_view />
			&nbsp;<br />
			<input type=hidden id=consultation_data />
			<input type=hidden id=default_date value="<?= date("Y-m-d",time()); ?>" />
			...
			<!--<input type='text' autocomplete="off" class=txtfield1 id=register_id name=register_id  style='width:90px; font-size:12px;' value='<?= @$cons['RegisterNumber'] ?>'/> -->
		<?php
			...
			//select all consultation that can be provided in the center
			...
			if($cons_list){
				if($cons_list[0]['ConsultationCategoryName'] == "invisible"){
					...
					foreach($cons_list as $cons){
						...
						$i++;
						echo "<label><input type=radio ".(( $cons2['ConsultationPriceID'] == $cons['ConsultationPriceID'] || ($i) == 0 || $cons['ConsultationCategoryName'] == "CPC Jour" || $cons['ConsultationCategoryName'] == $cat)?"checked":"")." name=consultation value='{$cons['ConsultationPriceID']}'>".($cons['ConsultationCategoryName'] != 'invisible'?$cons['ConsultationCategoryName']:"No consultation")."</label> ";
					}
				}
			} else{
...
<script>
	...
	$(document).ready(function(){
		$(".exam1").load("../rcp/adds-on/exam.php?number=1<?= $_exams_ != null?"&code=4":"" ?>&key=<?= $_GET['key'] ?>&source=labo&wght=<?= @$patient['Weight'] ?>");
		$(".medecine1").load("../rcp/adds-on/medecine.php?number=1<?= $_exams_ != null?"&code=4":"" ?>&key=<?= $_GET['key'] ?>&source=labo");
