...
<table style='' id=ex_table class=list-1 border=0>
	<tr >
		<td bgcolor='#e0e0e0' style='width:8%; border:0px solid #000; font-size:12px;'>
		...
		<td bgcolor='#e0e0e0' style='height:30px; width:300px; border:0px solid #000;'>
			<div style='max-height:45px; overflow:auto; width:100%; font-size:10px;' class='result_list<?php echo $_GET['number'] ?>'></div>
		</td>
		<td bgcolor='#e0e0e0' style='width:120px; border:0px solid #000;'>
...
<div style='border:0px solid #000; width:100%; text-align:left; min-height:1px;' class='exam<?php echo ($_GET['number'] + 1) ?>'>
	...
	<img style='cursor:pointer; position:relative; top:-45px; left:97%;' width='25px' style='' title='Add New Exam' src="../images/256.png" /></label>
</div>
...
<script>
	...
	$('#anotherexam').click(function(){
		$(".exam<?= ($_GET['number'] + 1) ?>").load("./adds-on/exam.php?number=<?php echo ($_GET['number'] + 1) ?><?= @$_GET['code'] == 2?"&code=21":"" ?><?= @$_GET['code'] == 4?"&code=4":"" ?>&key=<?= $_GET['key'] ?>&wght=<?= @$_GET['wght'] ?>");
		setTimeout(
			function(e){
...
if(@$_GET['code'] == 4 && $_GET['number'] <= count($_SESSION['exams'])){
	...
	//var_dump($_SESSION['exams'][($_GET['number'] - 1)]);
	?>
	<script>
