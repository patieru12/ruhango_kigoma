...
<script>
	...
	$("#examname<?php echo $_GET['number'] ?>").keyup(function(){
		...
		if($("#examname<?php echo $_GET['number'] ?>").val().replace(/ /g,"%20")){
			$(".result_list<?php echo $_GET['number'] ?>").load("./adds-on/ads_result.php?number=<?php echo $_GET['number'] ?>&exam=" + $("#examname<?php echo $_GET['number'] ?>").val().replace(/ /g,"%20") + "<?= @$_SESSION['exams'][($_GET['number'] - 1)]['Results']?"&rslt=".str_replace(" ","%20",$_SESSION['exams'][($_GET['number'] - 1)]['Results']):"" ?>");
	...
	function search(key, number){
		...
		$("#examresult" + number).val("");
			if(key.replace(/ /g,"%20"))
				$(".result_list" + number).load("./adds-on/ads_result.php?number=" + number + "&exam=" + key.replace(/ /g,"%20"));
			else
...
<table style='width:' id=ex_table class=list-1 border=0>
	<tr >
		<td bgcolor='#e0e0e0' style='width:80px; border:0px solid #000;'>
...
<div style='border:0px solid #000; width:1125px; text-align:left; min-height:1px;' class='exam<?php echo ($_GET['number'] + 1) ?>'>
	<label id= 'anotherexam'>
	<img style='cursor:pointer; position:relative; top:-10px; left:99%;' width='15px' style='' title='Add New Exam' src="../images/256.png" /></label>
</div>
...
<script>
	...
	$('#anotherexam').click(function(){
		$(".exam<?= ($_GET['number'] + 1) ?>").load("./adds-on/exam.php?number=<?php echo ($_GET['number'] + 1) ?><?= @$_GET['code'] == 2?"&code=21":"" ?><?= @$_GET['code'] == 4?"&code=4":"" ?>&key=<?= $_GET['key'] ?>");
	});
