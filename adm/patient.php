...
require_once "../lib2/cssmenu/adm_header.html";
?>
  <div id="w">
    <div id="content">
      <h1 style='margin-top:-55px'>Speed up System by eliminating unnecessary patients</h1>
      <b>
	  ...
	  <center>
	  <input type=text name=search_patient style='width:300px;' placeholder="Enter Insurance Card Number" <?php echo @$_GET['key']?"value='{$_GET['key']}'":"" ?> id=doc_search class=txtfield1 />
...
function findByInsurance(ins){
	...
	insurance = ins;
	$(".patient_found").load("mtn_patient.php?key=" + ins);
	//alert("FOUND");
...
$(document).ready(function(){
	...
	$("#insurance").change(function(e){
		$(".patient_selected").html("");
		$(".patient_found").load("mtn_patient.php?key="+$("#patient_search").val() + "&ins=" + $("#insurance").val());
	});
