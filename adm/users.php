...
if("adm" !== returnSingleField($sql="SELECT PostCode from sy_post WHERE PostID='{$_SESSION['user']['PostID']}'",$field="PostCode",$data=true, $con)){
	...
	if(@$_POST['update'] && is_numeric($_POST['id'])){
		...
		if(returnSingleField($sql="SELECT UserID FROM sy_users WHERE Phone='{$phone}' && UserID != '{$id}'",$field="UserID",$data=true, $con)){
			...
			//saveData("UPDATE md_price SET Status=0 WHERE MedecineNameID='{$name}' && Status=1",$con);
			//save new data
			if(saveData($sql="UPDATE sy_users SET  Name='{$name}', Phone='{$phone}', Password='{$password}', PostID='{$office}', CenterID={$center}, Status=1 WHERE UserID='{$id}'",$con)){
...
?>
  <div id="w">
    <div id="content">
	  ...
	  <?= @$data_to_update['UserID']?"<input type='text' name=id value='{$data_to_update['UserID']}' />":"" ?>
	  <table class=frm>
		<tr>
			<td>Name</td>
			<td>Phone Number</td>
