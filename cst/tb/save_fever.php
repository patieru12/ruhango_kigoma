...
if($_POST['tbRecordId']){
	...
	$fever 	= PDB($_POST['fever'], true, $con);
	$tbRecordId 	= PDB($_POST['tbRecordId'], true, $con);
	saveData("UPDATE tb_co_records SET fever ='{$fever}' WHERE id='{$tbRecordId}'",$con);
	echo "<span class=success>The Fever parameter received</span>";
} else {
	echo "<span class='error'>Fever Information can't be saved here</span>";
	return;
}
?>
