...
if(@$_POST['conf_diag'] ){
	...
	$add_cn .= "&& pa_records.PatientRecordID = co_records.PatientRecordID 
				...
				&& co_records.ConsultantID = '{$_SESSION['user']['UserID']}'";
	if($_POST['diagnostic'] != 'all'){
	...
	<?php
		...
		for($i = 0; $i<count($patient_records); $i++){
			...
			for($diag_counter=0; $diag_counter<$max_tr; $diag_counter++){
				...
				echo "<td>".$transfer_data[$patient_records[$i]['Status']]."</td>";
				$activeSheet->setCellValue(($first_column).$row, $transfer_data[$patient_records[$i]['Status']]);
