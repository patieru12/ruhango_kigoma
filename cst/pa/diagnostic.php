		...
		foreach($diagnostic AS $d){
			echo "
				<div style='padding-bottom:5px;'>
					<a style='color:blue; text-decoration:none;' title='Click to edit' href='' onclick='editDiagnostic(\"{$d['DiagnosticRecordID']}\", \"{$d['diagnosticName']}\"); return false;'>".$d['diagnosticName']."</a> | 
...
<script type="text/javascript">
	consultationID = <?= $consultation[0]['ConsultationRecordID'] ?>;
	function editDiagnostic(DiagnosticRecordID, Diagnostic){
	...
	function deleteDiagnostic(DiagnosticRecordID){
		if(DiagnosticRecordID){
			$.ajax({
				...
				data: "diagnosticRecordID=" + DiagnosticRecordID + "&url=ajax",
				cache: false,
