...
if(strlen($_GET['key']) > (@$_GET['ins'] == 1?11:($_GET['ins'] == 3?6:3))){
	...
	$_SESSION['patient_records'] = $patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT 	pa_records.*, 
																											mu_tm.*, 
																											pa_info.*, 
																											ad_village.*, 
																											ad_cell.*, 
																											...
																											pa_records.DateIn != '0000-00-00' && pa_records.InsuranceCardID LIKE('{$_GET['key']}%')
																											ORDER BY  pa_records.Status ASC, 
																													  DateIn DESC, 
																													  DocID ASC LIMIT 0, 20
