	  ...
	  <table border=0 class="a" style='background-color:#fff;'>
		...
		<tr>
			<td class=tm_view colspan="4">
				<label><input type=radio id='paid' name='tm' value='200' />TM Paid</label> <label><input type=radio id='not_paid' name='tm' value="0" />TM Not Paid</label>
			</td>
		</tr>
...
function OTHER(){
	$(".tm_view").html("<label><input type=radio id='paid' name='tm' value='<?= $tm_to_pay ?>' /> Paid <?= $tm_to_pay ?> RWF</label><input type=text placeholder='Receipt Number' id=receipt_number name=receipt_number class=txtfield1 style='width:160px;' />");
	$(".fcategory").html("");
	$(".affection_location").html("");
	$(".affection_locationdata").html("");
	$(".h_label").html("House Manager <label class=as_link onclick='$(\"#father\").val($(\"#name\").val())'>Self</label> <label class=error-text>*</label>");
