	...
	if(date("Y-m-d",time()) < $_POST['age']){
		...
		if($patient_id){
			if(!($record = returnSingleField("SELECT PatientRecordID FROM pa_records WHERE Status=0 && DateIn='{$_POST['date']}' && PatientID='{$patient_id}'",$field="PatientRecordID",$data=true, $con))){
				...
				//str_replace()
				$docnumber = preg_replace("/^\dA/", "", $docid);
				...
				// Add New Fields Here  <<<<<<<<----------------------------Removes a bit
				/*$sql .= ", lngth='".@$_POST['length']."'
						 , respiration='".@$_POST['Respiration']."'
						 , blooPressure='".@$_POST['Pressure']."'
						 , pulse='".@$_POST['Pulse']."'
						 , spo2='".@$_POST['SPO2']."'
			...
			die; */
				if($_POST['service']){
					...
					$consultations = formatResultSet($rslt=returnResultSet("SELECT 	a.*,
																					...
																					WHERE f.InsuranceNameID = '{$inNameID}' && 
																						  ...
																						  d.Status = 1 && 
																						  b.Status = 1 &&
																						  c.ConsultationCategoryName = '{$nightDefaultConsultation}'
																					", $con), true, $con);
				...
				//}
				$error = "<span class=success><b>New Patient Received <br />Code:{$docid} <br /><a href='./receipt.php?records=".$record."' id='print_file_now'>Print</a></b></span>";
				$error .= <<<SCRIPT
				<script>
					function click_now(){
						try{
