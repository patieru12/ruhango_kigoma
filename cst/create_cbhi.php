...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <h1>RSSB CBHI PATIENT MEDICAL DOCUMENT</h1>
	  ...
	  <table border=0 style='background-color:#fff;'>
		<tr>
			<td colspan=4><!--Insurance:<span class=error-text>*</span>-->
			<?php 
			/* if($insurance_all){
				$i=0; 
				foreach($insurance_all as $in){ 
		...
		</tr><tr><td><table border=0 style='width:200px; text-align:center; font-size:12px;'>
					<tr>
						<!-- <td style='width:50px;'>D</td><td>S</td> --><td style='width:100px;'>Insurance Card ID</td>
					</tr>
				</table>
			</td><td>Name:<span class=error-text>*</span></td><td>Age [<label class=as_link>yyyy-mm-dd or yyyy</label>]:</td><td>Sex<span class=error-text>*</span><!--<td>Family Chief ID Card:</td><td>Family Position:</td>-->
		</tr>
		<tr>
			<td>
			...
			<input id="patient_search" autocomplete="off" type=hidden name='card_id' class='txtfield1' style='' />
				<table border=0 style='width:200px; text-align:center;'>
					...
					<tr>
						...
						<td>/<input type=text class='txtfield1' autocomplete="off" id=se style='width:70%; font-size:12px;' /></td> -->
						<td style='width:100px;'>
							<!-- <input type=text autocomplete="off" class='txtfield1' id=pa style='width:85%; font-size:12px;' /> -->
							<input type=text autocomplete="off"  class='txtfield1' id=pa style='width:100%; font-size:16px;' />
						</td>
					</tr>
				</table>
		...
		<tr>
			...
			<td class=fcategorydata colspan=3>
				...
				<label style="cursor:pointer;"><input type=radio onclick="$('#fcategory').val(1)" name=cbhi_category value='1' id='cbhi_category1'>Category 1</label>
				<label style="cursor:pointer;"><input type=radio onclick="$('#fcategory').val(2)" name=cbhi_category value='2' id='cbhi_category2'>Category 2</label>
				<label style="cursor:pointer;"><input type=radio onclick="$('#fcategory').val(3)" name=cbhi_category value='3' id='cbhi_category3'>Category 3</label>
				<label style="cursor:pointer;"><input type=radio onclick="$('#fcategory').val(4)" name=cbhi_category value='4' id='cbhi_category4'>Category 4</label>
			</td>
		...
		<tr>
			...
			if($service){
				...
				foreach($service as $in){
					echo "<label style='padding:0 10px;'><input type=radio name='service' ".($in['ServiceCode'] == "CPC"?"checked":"")." ".($in['ServiceCode'] == "PST"?" rel=\"#overlay\" href=\"./pst_config.php?pst=check\" id=pst_ ":" class='uncheck_tm'")." value='{$in['ServiceNameID']}' id='{$in['ServiceCode']}'>{$in['ServiceCode']}</label>";
				} 
			} else{
		...
		<tr>
			<td><input type=text name='patientWeigth' autocomplete="off" id='weight' class='txtfield1' style='' /></td>
			<td><input type=text name='patientTemp' autocomplete="off" id='temperature' class='txtfield1' style='' /></td>
			<td><input type=text name='patientLength' autocomplete="off" id='length' class='txtfield1' style='' /></td>
			<td><input type=text name='patientMUAC' autocomplete="off" id='muac' class='txtfield1' style='' /></td>
		</tr>
		<tr>
			<td class=tm_view colspan=4 style='border:0px solid #000;'>
				<label>
					<input type=radio id='paid' name='tm' value='200' />TM Paid
				</label>
				<input type=text id=receipt_number name=receipt_number class=txtfield1 style='width:160px;' placeholder='Receipt Number' />
...
$(document).ready(function(){
	...
	$("#di").keyup(function(e){
		...
		$("#patient_search").val($("#di").val());
		//nextField("di",4,"se");
			
	});
	$("#se").keyup(function(e){
		//setTimeout(function(e){
		$("#patient_search").val($("#di").val() + "/" + $("#se").val());
		//nextField("se",2,"pa");
