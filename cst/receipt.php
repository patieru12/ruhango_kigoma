			...
			<tr>
				<td style="width: 20%;">
					...
					<div class=all_patient style='height:100%; width:350px; overflow:auto;'>
						<script>
							$(".all_patient").load("./patient-list-receipt.php<?= @$_GET['records']?'?records='.$_GET['records']:''; ?>");
						</script>
					</div>
				</td>
	...
	$(document).ready(function(){
		...
		$("#search").keyup(function(){
			...
			if(query_sent == false){
				...
				setTimeout(function(){
					if($("#search").val().trim != ""){
						$(".all_patient").load("./patient-list-receipt.php?filter=true&keyword=" + $("#search").val().replace(/ /g,"%20"));
						$("#bar").html("");
					}
					query_sent = false;
				},1000);
