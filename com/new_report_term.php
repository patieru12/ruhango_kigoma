...
?>
  <style>
	...
	<div id="content" style='height: auto;'>
		<h1 style='margin-top:-55px'>Items that should Appear on the daily reception summary</h1>
		<b>
		<?= $error ?>
		<?php
		$pp = null;
				...
				<td>
					...
					<div style='height:330px; overflow:auto;'>
						<form action="" method="POST">
							New Item<br />
							<input type="text" name="termName" value="" class="txtfield1" />
						</form>
					<?php
					$items = returnAllData($sql="SELECT * FROM sy_reception_report WHERE status=1 ORDER BY reportTerm DESC",$con);
					//var_dump($tarifs);
					if($items){
						foreach($items as $item){
							echo "<li>".$item['reportTerm']."</li>";
						}
					}
					$_GET['date'] = "2018-07-01";
					?>
					</div>
				</td>
				<td>
