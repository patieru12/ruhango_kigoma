...
if(strlen($_GET['key'])){
	...
	$patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT pa_records.DocID, pa_records.PatientRecordID, pa_records.FamilyCategory, pa_records.DateIn, pa_records.InsuranceNameID, pa_records.InsuranceCardID, se_name.ServiceCode, pa_info.*, ad_village.*, ad_cell.*, ad_sector.*, ad_district.* FROM pa_records, pa_info, se_name, se_records, ad_village, ad_cell, ad_sector, ad_district, sy_users, sy_center WHERE pa_info.PatientID = pa_records.PatientID && pa_info.VillageID=ad_village.ViillageID && ad_village.CellID=ad_cell.CellID && ad_cell.SectorID=ad_sector.SectorID && ad_sector.DistrictID=ad_district.DistrictID && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && {$sys} && DateIn LIKE('{$_GET['year']}-{$_GET['month']}%') && se_records.PatientRecordID = pa_records.PatientRecordID && se_records.ServiceNameID = se_name.ServiceNameID ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>Date</th><th>DocID</th><th>Service</th><th>Cat.</th><th>Insurance</th><th>Ins. Name</th><th>Age</th><th>Sex</th><th>Name</th><th>House Holder</th><th>Family Code</th><th colspan='2'></th><!--<th>Cons Cost</th><th>Lab</th><th>Imaging</th><th>Hosp.</th><th>Proc & Mat.</th><th>Consum.</th><th>Drugs.</th><th>Total</th><th>Dett. Fees</th><th>Total to be Paid</th>--></tr>
		<?php
		$t = array();
		$data = array();
		...
		for($i=0;$i<count($patients);$i++){
			...
			$display = "<td>{$r["DateIn"]}</td>
				<td>{$r['ServiceCode']}</td>
				<td>{$r['DocID']}</td>
				<td>".($r['FamilyCategory']>0?$r['FamilyCategory']:"")."</td>
				<td>{$r["InsuranceCardID"]}</td>
				<td>".returnSingleField("SELECT InsuranceName FROM in_name WHERE InsuranceNameID='".$r["InsuranceNameID"]."'","InsuranceName",true,$con)."</td>
				...
				$display .= "<td><a title='Click to Edit File' href='../rcp/reprint.php?key={$r['DocID']}&patientid={$r['PatientRecordID']}&rcv_patient=".sha1('receive_patient')."' style='color:blue; text-decoration:none;' target='_blank'><img src='../images/edit.png' /></a></td>";
				$display .= "<td><a href='#' onclick='if(confirm(\"Delete Completely the File For {$patients[$i]["Name"]} - {$r["InsuranceCardID"]} - {$r["DocID"]} From The System\")){ deleteProfileNow(\"{$r['PatientRecordID']}\"); } return false;' style='color:red; font-size:10px; font-weight:bold; text-decoration:none; float:right;' title='Delete The Patient File'><img src='../images/b_drop.png' /></a></td>";
				//$ddd[] = $tp_tot;
				if(!@$t[$t_counter])
					$t[$t_counter] = $tp_tot;
				else
	...
	function deleteProfileNow(record_id){
		...
		$.ajax({
			...
			success: function(result){
				...
				$(".patient_found").load("./empty_file_data_all.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val());
			}
		});
	}
