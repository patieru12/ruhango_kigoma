...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
																																												<!--	Total	85%-->
		<tr><th>No</th><th>Date</th><th>Service</th><th>Patient</th><th>House Holder</th><th>Consultation</th><th>Exams</th><th>Imaging</th><th>Hospitalisation</th><th>Acts</th><th>Consumables</th><th>Medicines</th><th>Others</th><th>Total</th><th>Total <?= abs(($insurance_info[0]['TypeofPayment'] == 1?100:0)-$insurance_info[0]['ValuePaid']) ?><?= ($insurance_info[0]['TypeofPayment'] == 1?"%":"") ?></th>
			<?php
		...
		$totalPrivateConsultationNumber = 0;
		$totalPrivateConsultationAmount = 0;

		$totalPrivateLaboratoryNumber = 0;
		$totalPrivateLaboratoryAmount = 0;

		$totalPrivateMedicinesNumber = 0;
		$totalPrivateMedicinesAmount = 0;

		$totalPrivateAccouchementNumber = 0;
		$totalPrivateAccouchementAmount = 0;

		$totalPrivateOtherActsNumber = 0;
		$totalPrivateOtherActsAmount = 0;

		$totalPrivateConsumablesNumber = 0;
		$totalPrivateConsumablesAmount = 0;

		$totalPrivateHospitalisationNumber = 0;
		$totalPrivateHospitalisationAmount = 0;
		...
		for($i=0;$i<count($patients);$i++){
			...
			echo "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				$conslt = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT co_price.Amount, co_records.ConsultationRecordID, co_records.status FROM co_records, co_price WHERE co_records.ConsultationPriceID = co_price. ConsultationPriceID && co_records.PatientRecordID='{$r['PatientRecordID']}'",$con),$multirows=true,$con);
				$cons = 0; $lab = 0; $md = 0; $tot = 0; $t_counter = 0; $other = 0;
				if($conslt){
					foreach($conslt as $c){
						$cons += $c['Amount'];
						if($c['status']){
							$totalPrivateConsultationNumber++;
							$totalPrivateConsultationAmount += $c['Amount'];
						}
						//select all exam related to this consultion record now
						$lab_exams = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT la_records.status, la_price.Amount, la_records.ExamRecordID FROM la_records, la_price WHERE la_records.ExamPriceID = la_price. ExamPriceID && la_records.ConsultationRecordID='{$c['ConsultationRecordID']}'",$con),$multirows=true,$con);
						if($lab_exams){
							foreach($lab_exams as $l){
								$lab += $l['Amount'];
								if($l['status']){
									$totalPrivateLaboratoryAmount += $l['Amount'];
									$totalPrivateLaboratoryNumber++;
								}
							}
						}
						//select all drugs related to this consultion record now
						$medecines = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT md_records.status, md_price.Amount, md_records.Quantity FROM md_records, md_price WHERE md_records.MedecinePriceID = md_price. MedecinePriceID && md_records.ConsultationRecordID='{$c['ConsultationRecordID']}'",$con),$multirows=true,$con);
						if($medecines){
							foreach($medecines as $l){
								$md += ($l['Amount'] * $l['Quantity']);
								if($l['status']){
									$totalPrivateMedicinesNumber++;
									$totalPrivateMedicinesAmount += $l['Amount'] * $l['Quantity'];
								}
							}
						}
					}
				...
				if(!@$t[$t_counter])
					...
					$t[$t_counter] += 0;
				$t_counter++;
				$ho = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT ho_record.status,ho_record.StartDate,ho_record.EndDate, ho_price.Amount, ho_record.Days FROM ho_record, ho_price WHERE ho_record.HoPriceID = ho_price. HOPriceID && ho_record.RecordID='{$r['PatientRecordID']}'",$con),$multirows=true,$con);
				$hosp = 0;
				if($ho){
					foreach($ho as $c){
						...
						$hosp += $cost_hosp;
						if($c['status']){
							$totalPrivateHospitalisationNumber++;
							$totalPrivateHospitalisationAmount += $cost_hosp;
						}
					}
				}
				echo "<td>{$hosp}</td>";
				...
				$act = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT ac_name.Name, ac_records.status, ac_price.Amount, ac_records.ActRecordID, ac_records.Quantity FROM ac_records, ac_price, ac_name WHERE ac_records.ActPriceID = ac_price.ActPriceID && ac_price.ActNameID = ac_name.ActNameID && ac_records.PatientRecordID='{$r['PatientRecordID']}'",$con),$multirows=true,$con);
				$acts = 0;
				if($act){
					foreach($act as $c){
						$acts += ($c['Amount']*$c['Quantity']);
						if($c['status']){
							if(preg_match("/^Acc./", $c['Name'])){
								$totalPrivateAccouchementNumber++;
								$totalPrivateAccouchementAmount += $c['Amount']*$c['Quantity'];
							} else{
								$totalPrivateOtherActsNumber++;
								$totalPrivateOtherActsAmount += $c['Amount']*$c['Quantity'];
							}
						}
					}
				}
				...
				$consum = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT cn_price.status, cn_price.Amount, cn_records.Quantity FROM cn_records, cn_price WHERE cn_records.MedecinePriceID = cn_price.MedecinePriceID && cn_records.PatientRecordID='{$r['PatientRecordID']}'",$con),$multirows=true,$con);
				$consumables = 0;
				if($consum){
					foreach($consum as $c){
						$consumables += ($c['Amount'] * $c['Quantity']);
						if($c['status']){
							$totalPrivateConsumablesNumber++;
							$totalPrivateConsumablesAmount += $c['Amount'] * $c['Quantity'];
						}
					}
				}
				...
				echo "<td align=right>".round($tot)."</td>";
				$ddd[] = $tot;
				if(!@$t[$t_counter])
					$t[$t_counter] = $tot;
				else
	...
	<?php
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='TM Paid' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='TM Paid', Number={$mmi_total_tm_number}, Amount='{$mmi_total_tm_amount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
		// saveData($SQL_INFO, $con);
	} else{
		$SQL_INFO = "UPDATE rpt_private SET itemName='TM Paid', Number={$mmi_total_tm_number}, Amount='{$mmi_total_tm_amount}', Date='{$dateRequest}' WHERE id={$dayInfoID}";
		// echo $SQL_INFO."<hr />";
		// saveData($SQL_INFO, $con);
	}
	...
	if($additionalCharges){
		foreach($additionalCharges AS $ad){
			...
			$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
			
			if(!$dayInfoID){
				$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
				// echo $SQL_INFO."<hr />";
				// saveData($SQL_INFO, $con);
			} else{
				$SQL_INFO = "UPDATE rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}' WHERE id={$dayInfoID}";
				// echo $SQL_INFO."<hr />";
				// saveData($SQL_INFO, $con);
			}
		}
	...
	// Here Save Those Provate Separated information
	$itemName = "Consultation";
	$itemNumber = $totalPrivateConsultationNumber;
	$itemamount = $totalPrivateConsultationAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	$itemName = "Laboratory";
	$itemNumber = $totalPrivateLaboratoryNumber;
	$itemamount = $totalPrivateLaboratoryAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	$itemName = "Medicines";
	$itemNumber = $totalPrivateMedicinesNumber;
	$itemamount = $totalPrivateMedicinesAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	$itemName = "Accouchement";
	$itemNumber = $totalPrivateAccouchementNumber;
	$itemamount = $totalPrivateAccouchementAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	$itemName = "Other Acts";
	$itemNumber = $totalPrivateOtherActsNumber;
	$itemamount = $totalPrivateOtherActsAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	$itemNumber = $totalPrivateConsumablesNumber;
	$itemamount = $totalPrivateConsumablesAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	$itemName = "Hospitalisation";
	$itemNumber = $totalPrivateHospitalisationNumber;
	$itemamount = $totalPrivateHospitalisationAmount;
	$dayInfoID = returnSingleField($sqlData = "SELECT id FROM rpt_private WHERE itemName='{$itemName}' && Date='{$dateRequest}'", "id", true, $con);
	if(!$dayInfoID){
		$SQL_INFO = "INSERT INTO rpt_private SET itemName='{$itemName}', Number={$itemNumber}, Amount='{$itemamount}', Date='{$dateRequest}'";
		// echo $SQL_INFO."<hr />";
	...
	if(count($data)>0){
		...
		<a href='#print_report.php?format=excel&in=corar' onclick="printExcel(); return false;"><img title="View in EXCEL" src="../images/excel.png" class=img_links width=40px /></a>
