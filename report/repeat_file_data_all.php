...
if(strlen($_GET['key'])){
	...
	$reference = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT se_name.ServiceCode, pa_records.PatientRecordID, in_name.InsuranceName, pa_records.DateIn, pa_records.InsuranceCardID FROM pa_records, se_records, in_name , se_name, sy_users, sy_center WHERE pa_records.InsuranceNameID = in_name.InsuranceNameID && pa_records.PatientRecordID = se_records.PatientRecordID && se_records.ServiceNameID = se_name.ServiceNameID && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && in_name.InsuranceName != 'Private' && {$sys} && DateIn LIKE('{$_GET['year']}-{$_GET['month']}%') ORDER BY pa_records.DateIn ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
	//var_dump($reference);
	/* echo $sql;  *///die;
	if($reference){
		...
		$printed_ids = array();
			...
			<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
				<tr><th>ID</th><th>Date</th><th>Service</th><th>DocID</th><th colspan=2>Insurance</th><th>Age</th><th>Sex</th><th>Name</th><th>House Holder</th><th>Family Code</th><th>Cons Cost</th><th>Lab</th><th>Imaging</th><th>Hosp.</th><th>Proc & Mat.</th><th>Consum.</th><th>Drugs.</th><th>Total</th><th>Dett. Fees</th><th>Total to be Paid</th></tr>
				<?php
		$rpt_counter = 0; $change = 1; $dspyd = false;
		if(!@$_SESSION['data_display']){
			foreach($reference as $ref){
				...
				$end_day = "";
				$select_data = true;
				$three_day = 60*60*24*2; //seconds of one days
				...
				$patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT se_name.ServiceCode, in_name.InsuranceName, pa_records.PatientRecordID, pa_records.DocID, pa_records.FamilyCategory, pa_records.DateIn, pa_records.InsuranceCardID, pa_info.*, ad_village.*, ad_cell.*, ad_sector.*, ad_district.* FROM pa_records, se_records, se_name, pa_info, ad_village, ad_cell, ad_sector, ad_district, sy_users, sy_center, in_name WHERE pa_records.InsuranceNameID = in_name.InsuranceNameID && pa_records.PatientRecordID = se_records.PatientRecordID && se_records.ServiceNameID = se_name.ServiceNameID && pa_info.PatientID = pa_records.PatientID && pa_info.VillageID=ad_village.ViillageID && ad_village.CellID=ad_cell.CellID && ad_cell.SectorID=ad_sector.SectorID && ad_sector.DistrictID=ad_district.DistrictID && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && {$sys} && in_name.InsuranceName != 'Private' && pa_records.InsuranceCardID='{$ref['InsuranceCardID']}' && DateIn >= '{$start_day}' && DateIn <= '{$end_day}' ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
				...
				if($patients){
						...
						for($i=0;$i<count($patients);$i++){
							...
							$display = "<td>{$r["DateIn"]}</td>
								...
								$display .= "<td>
								<a title='Click to Edit File' href='../rcp/reprint.php?key={$r['DocID']}&patientid={$r['PatientRecordID']}&rcv_patient=".sha1('receive_patient')."' style='color:blue; text-decoration:none;' target='_blank'>".round($tp_tot,0)."</a>
			...
			if(!$dspyd)
				echo "<tr><td colspan=21 align=center><span class=error-text>No Repeated Documents</span></td></tr>";
		}  else{
	...
	function deleteProfileNow(record_id,index_){
		...
		$.ajax({
			...
			success: function(result){
				...
				$(".patient_found").load("repeat_file_data_all.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val());
			}
		});
	}
