...
if(strlen($_GET['key'])){
	...
	$patients = formatResultSet($rslt=returnResultSet($sql="SELECT 	DISTINCT pa_records.DocID, 
																	...
																	ad_village.VillageName, ad_cell.CellID, ad_cell.CellName, 
																	ad_sector.SectorName,
																	ad_district.DistrictName, pa_info.* 
																	FROM 	se_name, 
																			se_records, 
																			pa_records, 
																			pa_info, 
																	...
																	WHERE pa_records.PatientRecordID = se_records.PatientRecordID && 
																		  ...
																		  ho_record.RecordID = pa_records.PatientRecordID &&
																		  se_name.ServiceCode != 'MAT' &&
																		  {$sys} && 
																		  DateIn LIKE('{$_GET['year']}-{$_GET['month']}%') {$sp_condition} 
																		  ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC
...
if($patients){
	...
	<script>
		$("#filter").click(function(e){
			$(".patient_found").load("hosp_data.php?key=" + $("#insurance").val() + "&day=" + $("#day").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + prompt("Enter Filter Key",'<?= @$_GET['filter'] ?>').replace(/ /g,"%20"));
			return e.preventDefault();
		});
		$("#filter_remove").click(function(e){
			$("#filter_").val("");
			$(".patient_found").load("hosp_data.php?key=" + $("#insurance").val() + "&day=" + $("#day").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + $("#filter_").val().replace(/ /g,"%20"));
			return e.preventDefault();
		});
		//$("#filter").focus();
	...
	<br />
	<?= $post ?> Hospitalisation Records<br />
	Date: <?= $_GET['month']."-".$_GET['year'] ?><br />
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>Date</th><th>Code Beneficiaire</th><th>Name</th><th>Chef de Famille</th><th>Address</th><th>Sex</th><th>Consul.<br />Diagnostic</th><!--<th>Labo</th><th>Price</th>--><th colspan=4>Medicines</th><th>Hosp</th><th>Price</th><th>Actes</th><th>Price</th><th>Consum.</th><th>Price</th><th>G. Total</th><!--<th>TM</th><th>Payable Total</th>--></tr>
		<tr><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><!--<th></th><th></th>--><th>Nature</th><th>QTY</th><th>UN.P</th><th>TOT.P</th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><!--<th></th><th></th>--></tr>
		<tbody>
		<?php
		//die;
		$t = array();
		$kk_ = "";
		$data = array();
		$data[] = array("ID","Date","Code Bebeneficiaire","Nom Beneficiaire","Chef de Famille", "Adress", "Gender","Consul.","Test Labo","Price","Medicines","Hosp.","Price","Act","Price","Consum","Price","General Total");
		...
		for($i=0;$i<count($patients);$i++){
			...
			$kk_ .=  "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				<td>".($r['VillageName'])."-".($r['CellName'])."-".($r['SectorName'])."</td>
				<td>".$r['Sex']."</td>
				";
			$ddd[] = $i+1;
			$ddd[] = $r['DateIn'];
			...
			$kk_ .= "<tr>
					...
					<a href='../rcp/reprint.php?key={$patients[$i]['DocID']}&patientid={$patients[$i]['PatientRecordID']}&rcv_patient=".sha1('rcv_patient')."' title='Click to Reprint!' target='_blank' style='color:blue; text-decoration:none;'>{$total}</a>
	...
	if(count($data)>0){
		...
		$_SESSION['header'] = array(
									...
									array("ADMINISTRATIVE DISTRICT"=>"HUYE", "Period"=>$_GET['day']."/".$_GET['month']."/".$_GET['year']),
									array("ADMINISTRATIVE SECTION"=>"HUYE"),
									array("HEALTH FACILITY"=>"SOVU HEALTH CENTER"),
									array("CODE / HEALTH FACILITY"=>"40211013"),
								);
		$_SESSION['report_title'] = array("title"=>"S U M M A R Y  OF (R S S B) / C B H I  P R E S T A T I O N");
		?>
		<style>
			.img_links{
				height:40px; 
		...
		<a href='./mat_data_print.php?format=excel&in=cbhi' target="_blank"><img title="View in EXCEL" src="../images/excel.png" class=img_links width=40px /></a>
		<?php
	}
