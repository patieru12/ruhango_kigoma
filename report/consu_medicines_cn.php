...
$report_title = ($_GET['medicine'] != "ALL"?$_GET['medicine']:"Consumables")." Distribution Report";
...
$sql = "SELECT DISTINCT cn_name.*, cn_price.* from cn_name, cn_price, cn_records, pa_records, sy_users, sy_center WHERE cn_records.PatientRecordID = pa_records.PatientRecordID && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && {$sys} && cn_name.MedecineNameID = cn_price.MedecineNameID  && cn_price.MedecinePriceID = cn_records.MedecinePriceID ORDER BY MedecineName ASC";
//echo $sql; echo "<br /><br />"; die;
if($_GET['medicine'] != "ALL")
	$sql = "SELECT DISTINCT cn_name.*, cn_price.* from cn_name, cn_price, cn_records, pa_records, sy_users, sy_center WHERE cn_records.PatientRecordID = pa_records.PatientRecordID && pa_records.ReceptionistID = sy_users.UserID && cn_name.MedecineName LIKE('%{$_GET['medicine']}%') && sy_users.CenterID = sy_center.CenterID && {$sys} && cn_name.MedecineNameID = cn_price.MedecineNameID && cn_price.MedecinePriceID = cn_records.MedecinePriceID  ORDER BY MedecineName ASC";
