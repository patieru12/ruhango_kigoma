...
if($patients){
	...
	<script>
		$("#filter").click(function(e){
			$(".patient_found").load("document_list_rama.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + prompt("Enter Filter Key",'<?= @$_GET['filter'] ?>').replace(/ /g,"%20"));
			return e.preventDefault();
		});
		$("#filter_remove").click(function(e){
			$("#filter_").val("");
			$(".patient_found").load("document_list_rama.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + $("#filter_").val().replace(/ /g,"%20"));
			return e.preventDefault();
		});
		//$("#filter").focus();
	...
	Private Patients<br />Prestation Document<br />
	Post: <?= $post ?><br />
	Date: <?= $_GET['year']."-".$_GET['month']."-".$_GET['day'] ?><br />
	...
	<div style='height:74%; margin-top:2px; width:100%; border:0px solid #000; overflow:auto;'>
	
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>Service</th><th>Code</th><th>Name</th><th>Chef de Famille</th><th>Consul.</th><th>Labo</th><th>Price</th><th colspan=4>Medicines</th><th>Hosp</th><th>Price</th><th>Actes</th><th>Price</th><th>Consum.</th><th>Price</th><th>G. Total</th><th>TM</th><th>Payable Total</th></tr>
		...
		for($i=0;$i<count($patients);$i++){
			...
			$tm = array('TicketPaid'=>0);//formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT mu_tm.TicketPaid FROM mu_tm WHERE mu_tm.PatientRecordID='{$r['PatientRecordID']}'",$con),$multirows=false,$con);
			...
			$kk_ .= "<tr>
					...
					<td class=number_right><a href='./reprint.php?key={$patients[$i]['DocID']}&patientid={$patients[$i]['PatientRecordID']}&rcv_patient=".sha1('rcv_patient')."' title='Click to Fill Other Information!' target='_blank' style='color:blue; text-decoration:none;'>{$total}</a>
	...
	if(count($data)>0){
		...
		$_SESSION['report_title'] = array("title"=>"S U M M A R Y  OF (R S S B) / R A M A  P R E S T A T I O N");
		?>
		<style>
			.img_links{
				height:40px; 
