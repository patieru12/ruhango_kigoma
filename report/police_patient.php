...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
																																												<!--	Total	85%-->
		<tr><th>No</th><th>Date</th><th>Service</th><th>Patient</th><th>House Holder</th><th>Consultation</th><th>Exams</th><th>Imaging</th><th>Hospitalisation</th><th>Acts</th><th>Consumables</th><th>Medicines</th><th>Others</th><th>Total</th><th>Policeman</th></tr>
		<?php
		$t = array();
		$data = array();
		
		$data[] = array("No","Service","No Police","Date","No du bénéficiaire","Nom et Prénom du bénéficiaire","Consultation","Examens","Imaging","Hospitalisation","Acte","Consumables","Medicaments","Autres","Total","Policeman");
		for($i=0;$i<count($patients);$i++){
			...
			echo "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				echo "<td align=right>{$patients[$i]['policeman']}</td>";
				$ddd[] = $patients[$i]['policeman'];
				if(!@$t[$t_counter])
					$t[$t_counter] = $tp_tot;
				else
					$t[$t_counter] += $tp_tot;
	...
	if(count($data)>0){
		...
		<a href='./print_report.php?format=excel&in=police' target="_blank"><img title="View in EXCEL" src="../images/excel.png" class=img_links width=40px /></a>
		<?php
	}
