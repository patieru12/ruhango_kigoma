...
$service = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT se_name.* FROM se_name, sy_users WHERE sy_users.UserID = se_name.DirectorID && sy_users.CenterID = '{$_SESSION['user']['CenterID']}' && se_name.Status=1 && se_name.ServiceCode='OSS' ORDER BY ServiceName ASC",$con),$multirows=true,$con);
$active = "other_service";
...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <div style='padding-bottom:10px; font-size:14px; display: none;'>
		<?php 
			if($insurance_all){
				$i=0; 
	  ...
	  <h1>CREATE PROFILE FOR OTHER SERVICE USE (this profile will not be able to receive medical service)</h1>
	  ...
	  <table border=0 class="a" style='background-color:#fff;'>
		<tr>
			...
			Date: <input id="date" type=text name='date' onblur='' class='txtfield1' onclick="ds_sh(this,'date')" style='width:150px;' value='<?php echo date("Y-m-d",time()) ?>' />
			</td>
		...
		<tr>
			<td>Name:<span class=error-text>*</span></td>
			<td style="display: none;">Age [<label class=as_link>yyyy-mm-dd or yyyy</label>]:</td>
			<td style="display: none;">Sex<span class=error-text>*</span><!--<td>Family Chief ID Card:</td><td>Family Position:</td>-->
		</tr>
		<tr>
			<td><input id="patient_search" autocomplete="off" type=hidden name='card_id' class='txtfield1' style='' /><input type=text name='name' autocomplete="off" onkeyup='if(familyChief()){$("#father").val($("#name").val());}' id=name class='txtfield1' style='' /></td>
			<td style="display: none;"><input type=text name='age' autocomplete="off" id=age class='txtfield1' onclick="ds_sh(this,'age')" style='' /></td>
			<td style="display: none;"><label><input type=radio name=sex value="Female" required id=female>Female</label> <label><input type=radio name=sex value="Male" required id=male>Male</label> <label><input type=radio name=sex value="Not Specified" required id=not_specified checked>Male</label></td>
			
		</tr>
		<tr style="display: none;"><td class=h_label>House Manager:</td></td><td class=fcategory>Category<span class=error-text>*</span></td></td><td class=affection_location></td><td class=sex></td>
		</tr>
		...
		<tr>
			<td colspan="e">
				<?php
					...
					if(!$activeDate){
						echo "<span class='error'>No Service can be provided</span>";
						return;
					}
					...
					$sy_price = formatResultSet($rslt=returnResultSet("SELECT 	a.TarifID AS ProductPriceID,
																				...
																				WHERE a.InsuranceNameID = '{$insurance[0]['InsuranceNameID']}' &&
																					  a.Date = '{$activeDate}'
																				", $con), true, $con);
				...
				<div id="TotalServicePayment" style="font-size:18px">
					Total to pay: <span id='amountOnly'>0</span> RWF
				</div>
			</td>
		</tr>
		<tr style="display: none;">
			...
			if($service){
				$i=0; 
				foreach($service as $in){ echo "<label style='padding:0 10px;'><input type=radio name='service' ".($in['ServiceCode'] == "OSS"?"checked":"")." value='{$in['ServiceNameID']}' id='{$in['ServiceCode']}'>{$in['ServiceCode']}</label>"; } 
			} else{
		...
		<tr>
			<td colspan=4 align=center>
				...
				<input type=hidden name=next_frequency id=next_frequency value='1' />
				<input type=hidden name=document_list id=document_list value='' />
				<label style="display: none;"><input type=checkbox id=update disabled> Update</label>
				<input type=submit id=save class="flatbtn-blu" name=rcv_patient value='Save' />
				<input type=reset class="flatbtn-blu" onclick='window.location="./create_other.php";' value='Clear' />
			</td>
		</tr>
	  </table>
...
<script>
	function calculateTotal(newValue, operation){
		var totalAmount = $("#totalAmount").val()*1;
		if(operation == "add"){
			totalAmount += (newValue*1);
		} else{
			totalAmount -= (newValue*1);
		}

		$("#totalAmount").val(totalAmount);
		$("#amountOnly").html(totalAmount);
	}
...
$(document).ready(function(){
	...
	$('#save').click(function(e){ 
			...
			var amountTopay = $("#amountOnly").html()*1;
			if(amountTopay <= 0){
				$("#upload_out").html('<span class="error-text">No thing is selected.</span>');
			}else if(!confirm("You Received " + $("#amountOnly").html() + " RWF in the cashbox\nContinue by printing the receipt!\nTHANKS")){
				$("#upload_out").html('<span class="error-text">The Operation cancelled.</span>');
			} else{
				$("#upload_out").html('');
