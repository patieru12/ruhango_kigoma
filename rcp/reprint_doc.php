...
if(strlen($_GET['key']) || 1){
	...
	$records = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT 	pa_records.*, 
																			...
																			WHERE pa_info.PatientID=pa_records.PatientID && 
																				  mu_tm.PatientRecordID = pa_records.PatientRecordID && 
																				  pa_records.VillageID = ad_village.ViillageID && 
	...
	<table class=list id=vsbl border="1" style='width:100%;'>
		<tr><th colspan=2>&nbsp;</th><th>#</th><th>Date</th><th>Service</th><th>TM</th><th>Insurance</th><th>Card</th><th>Cat</th><th>Name</th><th>Age</th><th>Sex</th><th>Family</th><th>Phone</th><th>Weight</th><th>Temp.</th><th>District</th><th>Sector</th><th>Cell</th><th>Village</th></tr>
		<?php
		$service = null;
		$k = 0;
		for($i=0;$i<count($records);$i++){
			...
			echo "<tr id='id{$i}'>
					...
					<td class='pa_{$i}11' onclick='if(true){ return false;} if($(\"#edit_mode\").val() == 0){ edit_function(\"pa_{$i}11\",\"{$records[$i]['InsuranceCardID']}\",\"pa_records\",\"{$records[$i]['PatientRecordID']}\",\"PatientRecordID\",\"InsuranceCardID\",\"save_rqst/save_pa_info.php\");}'>{$records[$i]["InsuranceName"]}</td>
					...
					<td class='pa_{$i}10' onclick='if(true){ return false;} if($(\"#edit_mode\").val() == 0){ edit_function(\"pa_{$i}10\",\"{$records[$i]['TicketPaid']}\",\"mu_tm\",\"{$records[$i]['TicketID']}\",\"TicketID\",\"TicketPaid\",\"save_rqst/save_pa_info.php\");}'>".($records[$i]["InsuranceName"] == "CBHI"?$records[$i]["FamilyCategory"]:"")."</td>
					...
					<td class='pa_{$i}12' onclick='if($(\"#edit_mode\").val() == 0){ edit_function(\"pa_{$i}12\",\"{$records[$i]['phoneNumber']}\",\"pa_info\",\"{$records[$i]['PatientID']}\",\"PatientID\",\"phoneNumber\",\"save_rqst/save_pa_info.php\");}'>{$records[$i]["phoneNumber"]}</td>
