...
if(strlen($_GET['key'])){
	...
	$patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT pa_records.*, pa_records.FamilyCategory, pa_records.DateIn, pa_records.InsuranceCardID, pa_info.* FROM pa_records, pa_info, sy_users, sy_center WHERE pa_info.PatientID = pa_records.PatientID  && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && {$sys} && InsuranceNameID='{$_GET['key']}' && DateIn LIKE('{$_GET['year']}-{$_GET['month']}-{$_GET['day']}') {$sp_condition} ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
...
if($patients){
	...
	<script>
		$("#filter").click(function(e){
			$(".patient_found").load("patient_list_cbhi.php?key=" + $("#insurance").val() + "&day=" + $("#day").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + prompt("Enter Filter Key").replace(/ /g,"%20"));
	...
	<div style='max-height:350px; max-width:850px; border:0px solid #000; overflow:auto;'>
	<a href='filter.php' id=filter style='float:right; border:1px solid #eee; font-size:12px;' >Filter</a>
	
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>TM</th><th>Cat.</th><th>Insurance</th><th>Age</th><th>Sex</th><th>Name</th><th>House Holder</th><th>Family Code</th></tr>
		<?php
		$t = array();
		$kk_ = "";
		$data = array();
		...
		for($i=0;$i<count($patients);$i++){
			...
			$kk_ .=  "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				<td>".($i+1)."</td>
				<td class='edit{$i}1' onclick='if($(\"#edit_mode\").val() == 0){edit_function(\"edit{$i}1\",\"{$tm_p}\",\"mu_tm\",\"{$tm_i}\",\"TicketID\",\"TicketPaid\");}'>".$tm_p."</td>
				<td class='edit{$i}2' onclick='if($(\"#edit_mode\").val() == 0){edit_function(\"edit{$i}2\",\"{$r['FamilyCategory']}\",\"pa_records\",\"{$r['PatientRecordID']}\",\"PatientRecordID\",\"FamilyCategory\");}'>{$r['FamilyCategory']}</td>
