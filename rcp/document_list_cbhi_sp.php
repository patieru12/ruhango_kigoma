...
if(strlen($_GET['key'])){
	...
	$patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT se_name.ServiceCode, mu_tm.*, pa_records.DocID, pa_records.PatientRecordID, pa_records.FamilyCategory, pa_records.DateIn, pa_records.InsuranceCardID, pa_info.*, ad_village.*, ad_cell.*, ad_sector.*, ad_district.* FROM mu_tm, pa_records, se_records, se_name, pa_info, ad_village, ad_cell, ad_sector, ad_district, sy_users, sy_center WHERE pa_records.PatientRecordID = mu_tm.PatientRecordID ".($_GET['sp'] != "all"?"&& mu_tm.Type='{$_GET['sp']}'":"")." && pa_records.PatientRecordID = se_records.PatientRecordID && se_records.ServiceNameID = se_name.ServiceNameID && pa_info.PatientID = pa_records.PatientID && pa_info.VillageID=ad_village.ViillageID && ad_village.CellID=ad_cell.CellID && ad_cell.SectorID=ad_sector.SectorID && ad_sector.DistrictID=ad_district.DistrictID && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && {$sys}  && InsuranceNameID='{$_GET['key']}' && DateIn LIKE('{$_GET['year']}-{$_GET['month']}-{$_GET['day']}') {$sp_condition} ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
...
if($patients){
	...
	<script>
		$("#filter").click(function(e){
			$(".patient_found").load("document_list_cbhi_sp.php?key=" + $("#insurance").val() + "&day=" + $("#day").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&sp=<?= $_GET['sp'] ?>" + "&filter=" + prompt("Enter Filter Key",'<?= @$_GET['filter'] ?>').replace(/ /g,"%20"));
			return e.preventDefault();
		});
		$("#filter_remove").click(function(e){
			$("#filter_").val("");
			$(".patient_found").load("document_list_cbhi_sp.php?key=" + $("#insurance").val() + "&day=" + $("#day").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&sp=<?= $_GET['sp'] ?>" + "&filter=" + $("#filter_").val().replace(/ /g,"%20"));
			return e.preventDefault();
		});
		//$("#filter").focus();
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>Service</th><th>Code Beneficiaire</th><th>Name</th><th>Chef de Famille</th><th>Consul.</th><th>Labo</th><th>Price</th><th colspan=4>Medicines</th><th>Hosp</th><th>Price</th><th>Actes</th><th>Price</th><th>Consum.</th><th>Price</th><th>G. Total</th><th>TM</th><th>Payable Total</th></tr>
		...
		$kk_ = "";
		$data = array();
		$data[] = array("ID","Code Beneficiary","Nom Beneficiary","Chef de Famille","Consul.","Test Labo","Price","Medicines","Hosp.","Price","Act","Price","Consum","Price","General Total","TM","Payable Total");
		...
		for($i=0;$i<count($patients);$i++){
			...
			$kk_ .= "<tr>
					...
					<a href='./reprint.php?key={$patients[$i]['DocID']}&patientid={$patients[$i]['PatientRecordID']}&rcv_patient=".sha1('rcv_patient')."' title='Click to Reprint!' target='_blank' style='color:blue; text-decoration:none;'>{$total}</a>
	...
	if(count($data)>0){
		...
		<a href='./print_report_pres.php?format=excel&in=cbhi&sh_name=<?= $_GET['year']."-".$_GET['month']."-".$_GET['day']; ?>' target="_blank"><img title="View in EXCEL" src="../images/excel.png" class=img_links width=40px /></a>
		<?php
	}
