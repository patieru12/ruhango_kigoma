...
//die;
//var_dump($_SESSION);
$insurance = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT in_name.* from in_name, in_category, in_forms, in_price WHERE in_name.CategoryID=in_category.InsuranceCategoryID && in_name.InsuranceNameID = in_forms.InsuranceNameID && in_name.InsuranceName='SAN Fransisco' && in_name.InsuranceNameID = in_price.InsuranceNameID ORDER BY InsuranceCode ASC, InsuranceName DESC",$con),$multirows=true,$con);
...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <h1>SAN FRANSISCO PATIENT MEDICAL DOCUMENT</h1>
	  <form method=post action="rcv_patient_san_fransisco.php" id='rcv_patient_frm'>
	  ...
	  <table border=0 class="a" style='background-color:#fff;'>
		...
		<tr>
			<td class="found_link" colspan=6></td>
		</tr>
		<tr class="patient_found_search">
			<td colspan="6">
				<a href='./force-selection-san-fransisco.php' id=display_now rel="#overlay"><img src='../images/view.png' style='width:0px;' /></a>
				<div class="patient_found"></div>
			</td>
		</tr>
		<tr><td>Key Population:<span class=error-text>*</span></td><td>Age [<label class=as_link>yyyy-mm-dd or yyyy</label>]:</td><td>Sex<span class=error-text>*</span><!--<td>Family Chief ID Card:</td><td>Family Position:</td>-->
		</tr>
		<tr>
			<td>
				<input id="patient_search" autocomplete="off" type=hidden name='card_id' class='txtfield1' style='' />
				<input type=text name='name' autocomplete="off" onkeyup='if(familyChief()){$("#father").val($("#name").val());}' id=name class='txtfield1' style='' />
			</td>
			<td>
				<input type=text name='age' autocomplete="off" id=age class='txtfield1' onclick="ds_sh(this,'age')" style='' />
			</td>
			<td>
				<label><input type=radio name=sex value="Female" required id=female>Female</label> <label><input type=radio name=sex value="Male" required id=male>Male</label>
			</td>
			
		</tr>
		<tr><td class=h_label>***********</td></td><td class=fcategory>Category<span class=error-text>*</span></td></td><td class=affection_location></td><td class=sex></td>
		</tr>
		<tr>
...
function OTHER(){
	...
	$(".affection_location").html("");
	$(".affection_locationdata").html("");
	$(".h_label").html("**************** <label class=as_link onclick='$(\"#father\").val($(\"#name\").val())'>Self</label> <label class=error-text>*</label>");
	$(".sex").html("");
	$(".sexdata").html("");
}
OTHER();
