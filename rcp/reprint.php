...
<script>
function receivePatient(id, checked_data, service_code='', auto=false){
	...
	if(!auto){
		...
		$(".doc_found").load("reprint_doc_2.php?key="+$("#doc_search").val().replace(/ /g,"%20"));
	}
	//alert("Function Called");
	setTimeout(function(e){
		$(".doc_selected").load("receive_patient_" + service_code + ".php?displ=true&key=" + id );
	}, 200);
}
$(document).ready(function(){
	$("#doc_search").focus();
