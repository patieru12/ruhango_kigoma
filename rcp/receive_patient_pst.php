...
$cons2 = $cons = formatResultSet($rslt=returnResultSet($sql="SELECT co_category.*, co_price.ConsultationPriceID, co_records.* FROM co_category, co_price, co_records WHERE co_records.ConsultationPriceID = co_price.ConsultationPriceID && co_category.ConsultationCategoryID = co_price.ConsultationCategoryID && co_price.InsuranceCategoryID = '{$insurance_category_id}' && co_records.PatientRecordID='{$patient['PatientRecordID']}' ORDER BY ConsultationCategoryName ASC",$con),$multirows=false,$con);
...
<div id=acc_ribbon>
	<label><input type=radio name=pst onclick='pansement("travail","<?= @$patient['Weight']; ?>",true)' />Travail d' Accouchement</label>
	<label><input type=radio name=pst onclick='pansement("acc. eutoc. sans episio","<?= @$patient['Weight']; ?>",true)' />Eutocique Sans Episio</label>
	<label><input type=radio name=pst onclick='pansement("acc. eutoc. avec episio","<?= @$patient['Weight']; ?>",true)' />Eutocique Avec Episio</label>
	<label><input type=radio name=pst onclick='pansement("acc. dystocique","<?= @$patient['Weight']; ?>",true)' />Dystocique</label>
</div>-->
&nbsp;
...
<form action="./save_from_cons_doc.php" method=post style='border:1px solid #efefef;' id="frm_data">
	...
	<div class=bb>
	<input type=button id=save_data_and_print name=print_bill style='font-size:8px; display:none;' class=flatbtn value='Save' />
	...
	<table class=frm border=0>
		...
		<th>
			...
			<input type='hidden' autocomplete="off" class=txtfield1 id=register_id name=register_id  style='width:90px; font-size:12px;' value='<?= @$cons['RegisterNumber'] ?>'/>
		<?php
