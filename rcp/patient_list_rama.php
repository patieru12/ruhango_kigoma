...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:70%; font-size:30px;'>
		<tr><th>ID</th><th>Affiliation Number</th><th>Age</th><th>Sex</th><th>Name</th><th>Affiliate Name</th><th>Affectation</th></tr>
		<?php
		$t = array();
		$kk_ = "";
		$data = array();
		...
		for($i=0;$i<count($patients);$i++){
			...
			$kk_ .=  "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				<td>{$patients[$i]["AffiliateNumber"]}</td>";
