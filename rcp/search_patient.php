...
if(strlen($_GET['key']) > (@$_GET['ins'] == 1?11:($_GET['ins'] == 3?6:3))){
	...
	$_SESSION['patient_records'] = $patients = $records = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT pa_records.*, mu_tm.*, pa_info.*, ad_village.*, ad_cell.*, ad_sector.*, ad_district.*  from pa_records, mu_tm, pa_info, ad_village, ad_cell, ad_sector, ad_district WHERE pa_info.PatientID=pa_records.PatientID && mu_tm.PatientRecordID = pa_records.PatientRecordID && pa_info.VillageID=ad_village.ViillageID && ad_village.CellID=ad_cell.CellID && ad_cell.SectorID=ad_sector.SectorID && ad_sector.DistrictID=ad_district.DistrictID && pa_records.DateIn != '0000-00-00' && (pa_records.DocID LIKE('%{$_GET['key']}%') || pa_records.InsuranceCardID LIKE('%{$_GET['key']}%') || pa_info.Name LIKE('%{$_GET['key']}%')) ORDER BY  pa_records.Status ASC, DateIn DESC, DocID ASC LIMIT 0, 20 ",$con),$multirows=true,$con);
	//echo "OK";
	if($patients){
		//echo count($patients);
		?>
