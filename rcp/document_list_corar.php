...
if($patients){
	...
	<script>
		$("#filter").click(function(e){
			$(".patient_found").load("document_list_corar.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + prompt("Enter Filter Key",'<?= @$_GET['filter'] ?>').replace(/ /g,"%20"));
			return e.preventDefault();
		});
		$("#filter_remove").click(function(e){
			$("#filter_").val("");
			$(".patient_found").load("document_list_corar.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + $("#filter_").val());
			return e.preventDefault();
		});
		//$("#filter").focus();
		function deleteProfileNow(record_id){
			$.ajax({
				...
				success: function(result){
					...
					$(".patient_found").load("document_list_corar.php?key=" + $("#insurance").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val() + "&filter=" + $("#filter_").val().replace(/ /g,"%20"));
				}
			});
		}
	</script>
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>Date</th><th>Service</th><th>Police No</th><th>Name</th><th>N<sup>o</sup> de Police</th><th>Consul.</th><th>Labo</th><th>Price</th><th colspan=4>Medicines</th><th>Hosp</th><th>Price</th><th>Actes</th><th>Price</th><th>Consum.</th><th>Price</th><th>G. Total</th><th>TM</th><th>Payable Total</th></tr>
