...
?>
  ...
  <div id="w">
    <div id="content">
      <h1 style='margin-top:-55px'>Document List CBHI</h1>
      <b>
...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <table class=list-1 style='display:block;'><tr><td>Post</td><td>Year</td><td>Month</td><td>Day</td></tr>
	  <tr>
	  <td>
	  <label><input type=checkbox checked checked onclick='if($("#post_sovu").prop("checked")){$("#post").val($("#post").val() + "_1")} else{ $("#post").val($("#post").val().replace("_1","")) }' id=post_sovu name=post value=1>Busoro</label> 
...
$(document).ready(function(){
	...
	$("#generate").click(function(e){
		...
		$(".patient_found").load("document_list_cbhi_sp.php?key=" + $("#insurance").val() + "&day=<?= $_GET['day'] ?>&month=<?= $_GET['month'] ?>&year=<?= $_GET['year'] ?>&post=" + $("#post").val() + "&sp=<?= $_GET['sp'] ?>&filter=" + $("#filter_").val());
	});
	$("#generate").click();
