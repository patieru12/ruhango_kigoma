...
<br /><!--
<div id=pst_ribbon style='text-align:center; font-size:12px;'>
	<label><input type=radio name=pst onclick='pansement("pansement simple")' />pst simple</label>
	<label><input type=radio name=pst onclick='pansement("pansement compliqué")' />pst complique</label>
	<label><input type=radio name=pst onclick='pansement("suture simple","<?= @$patient['Weight']; ?>")' />suture simple</label>
	<label><input type=radio name=pst onclick='pansement("suture compliqué","<?= @$patient['Weight']; ?>")' />suture complique</label>
</div>-->
<div id=acc_ribbon style='text-align:left; font-size:14px; margin-top:10px;'>
	<label style='padding-right:10px;'><input type=radio name=pst onclick='pansement("travail","<?= @$patient['Weight']; ?>",true)' />Travail d' Accouchement</label>
	<label style='padding-right:10px;'><input type=radio name=pst onclick='pansement("acc. eutoc. sans episio","<?= @$patient['Weight']; ?>",true)' />Eutocique Sans Episio</label>
	<label style='padding-right:10px;'><input type=radio name=pst onclick='pansement("acc. eutoc. avec episio","<?= @$patient['Weight']; ?>",true)' />Eutocique Avec Episio</label>
	<label style='padding-right:10px;'><input type=radio name=pst onclick='pansement("acc. dystocique","<?= @$patient['Weight']; ?>",true)' />Dystocique</label>
</div>
&nbsp;
...
<form action="./save_from_cons_doc.php" method=post style='border:1px solid #efefef;' id="frm_data">
	...
	<div style='height:1px; border:0px solid #000;'>
		<input type=text class=txtfield1 onblur='setTimeout("ds_hi()",200);' style='width:75px; font-size:12px; position:relative; top: 18px; left:18px;' name=consultation_date id=consultation_date onclick='ds_sh(this,"consultation_date")' value='<?= $patient['DateIn']?$patient['DateIn']:date("Y-m-d",time()) ?>' />
	</div>
	<table class=frm border=0>
			...
			if($cons_list){
				if($cons_list[0]['ConsultationCategoryName'] == "invisible"){
					...
					foreach($cons_list as $cons){
						...
						$i++;
						echo "<label><input type=radio ".(( $cons2['ConsultationPriceID'] == $cons['ConsultationPriceID'] || ($i) == 0 || $cons['ConsultationCategoryName'] == "invisible" || $cons['ConsultationCategoryName'] == $cat)?"checked":"")." name=consultation value='{$cons['ConsultationPriceID']}'>".($cons['ConsultationCategoryName'] != 'invisible'?$cons['ConsultationCategoryName']:"No consultation")."</label> ";
					}
				}
			} else{
		...
		<tr><td>Diagnostic</td><td>
			<table class=list-1>
				...
				<tr>
					<td colspan=2 class='_some_data' style='font-size:12px;'>
						<label><input type=radio required onclick='' id='nopecime' checked name=pecime value='0'>NO PECIME</label>
						<label><input type=radio required onclick='' id='pecime' name=pecime value='1'>PECIME</label>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
