...
if(strlen($_GET['key']) ){
	...
	$sql = "SELECT 	a.*,
					...
					g.AffiliateNumber AS AffiliateNumber,
					g.AffiliateName AS AffiliateName,
					g.Affectation AS Affectation,
					COALESCE(b.applicationNumber, '') AS applicationNumber,
					...
					INNER JOIN pa_insurance_cards AS g
	...
	/*if($patients[0]['InsuranceNameID'] != $InsuranceNameID){
		...
		echo "<span class=error-text>No Match Found ".strlen($_GET['key'])."</span>";
	}
	?>
	<script>
