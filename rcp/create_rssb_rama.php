...
//die;
//var_dump($_SESSION);
$insurance = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT in_name.* from in_name, in_category, in_forms, in_price WHERE in_name.CategoryID=in_category.InsuranceCategoryID && in_name.InsuranceNameID = in_forms.InsuranceNameID && in_name.InsuranceName='RSSB RAMA' && in_name.InsuranceNameID = in_price.InsuranceNameID ORDER BY InsuranceCode ASC, InsuranceName DESC",$con),$multirows=true,$con);
...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <h1>RSSB RAMA PATIENT MEDICAL DOCUMENT</h1>
	  <form method=post action="rcv_patient_rssbrama.php" id='rcv_patient_frm'>
	  ...
	  <table border=0 class="a" style='background-color:#fff;'>
		<tr>
			...
			Consultation Date: <input id="date" type=text name='date' onblur='setTimeout("ds_hi()",1000)' class='txtfield1' onclick="ds_sh(this,'date')" style='width:150px;' value='<?php echo date("Y-m-d",time()) ?>' />
			</td>
			<td colspan="2">
				Serial Nr: <input id="DailyNumberID" type=text name='DailyNumberID' class='txtfield1' style='width:200px;' placeholder="Hard copy Serial Number" />
		...
		<tr class="patient_found_search">
			<td colspan="6">
				<a href='./force-selection-rama.php' id=display_now rel="#overlay"><img src='../images/view.png' style='width:0px;' /></a>
		...
		<tr>
			<td><input id="patient_search" autocomplete="off" type=text name='card_id' class='txtfield1' style='' value="" /></td>
		...
		<tr>
			<td>Affiliate<span class=error-text>*</span></td>
			<td class=h_label>House Manager:</td>
			<td class=fcategory>Category<span class=error-text>*</span></td>
			<td class=affection_location></td>
			<td class=sex></td>
		</tr>
		<tr>
			<td>
				<label class=as_link_ onclick='$("#father").val($("#name").val()); $("#rama_affiliate").val("Self")'><input type=radio id="Self" name='ralation'>Self</label>
				<label class=as_link_ onclick='$("#father").val(""); $("#rama_affiliate").val("Conjoint")'><input type=radio id="Conjoint" name='ralation'>Conjoint</label>
				<label class=as_link_ onclick='$("#father").val(""); $("#rama_affiliate").val("Parent")'><input type=radio id="Parent" name='ralation' value="3">Parent</label>
		...
		<tr>
			<td><input type=text name='phoneNumber' autocomplete="off" id=phoneNumber class="txtfield1" style='' /></td>
			<td><input type=text name='nationalIdentity' autocomplete="off" id=nationalIdentity class="txtfield1" style='' /></td>
		</tr>
		<tr>
		...
		<tr>
			<td class=tm_view>
				<label>
					<input type=radio id='paid' name='tm' value='200' />TM Paid
					</label>
					<input type=text id=receipt_number name=receipt_number class=txtfield1 style='width:70px;' />
					<label><input type=radio id='not_paid' name='tm' value="0" />TM Not Paid</label></td>
		</tr>
		...
		<input type=submit id=save class="flatbtn-blu" name=rcv_patient value='Save' /> <input type=reset class="flatbtn-blu" onclick='window.location="./create_rssb_rama.php";' value='Clear' /></td></tr>
	  </table>
...
$(document).ready(function(){
		...
		if(e.which == 13 ){
			...
			if(docsearch ==false){
				...
				setTimeout(function(e){
					$(".patient_found").load("search_patient_rama.php?key="+$("#DailyNumberID").val().replace(" ","_") + "&ins=" + $("#insurance").val() + "&date=" + $("#date").val());
	...
	$("#patient_search").keyup(function(e){
		...
		$(".patient_found").load("search_patient_rama.php?key="+$("#patient_search_").val().replace(/ /g,"_") + "&ins=" + $("#insurance").val() + "&date=" + $("#date").val());
		return e.preventDefault();
	});
