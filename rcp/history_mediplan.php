...
?>
  ...
  function edit_function(cl,ex_val,tbl,ref_val,ref_field, fld){
	  $("." + cl).html("<input id=focus_now onclick='' onblur='save_request(\""+ tbl +"\",\""+ fld +"\",\""+ ref_val +"\",\""+ ref_field +"\");' type=text value='" + ex_val + "' />");
	  $("#focus_now").focus();
	  $("#edit_mode").val("1");
  }
  </script>
  <div id="w">
    <div id="content">
      <h1 style='margin-top:-55px'>Patient List MEDIPLAN</h1>
      <b>
...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <input type=hidden name=post_ value='' id=post />
	  <table class=list-1><tr><td>Post</td><td>Year</td><td>Month</td><td>Day</td></tr>
	  <tr>
	  <td>
	  <label><input type=checkbox onclick='if($("#post_sovu").prop("checked")){$("#post").val($("#post").val() + "_1")} else{ $("#post").val($("#post").val().replace("_1","")) }' id=post_sovu name=post value=1>Busoro</label> 
	 ...
	 <div id="ds" style='min-width:100px'></div></td></tr></table>
	  ...
	  <div class="patient_found" style='max-height:430px;'>
		<span class=error-text>Select Post to view MEDIPLAN Patient History</span>
	  </div>
...
$(document).ready(function(){
	...
	$("#generate").click(function(e){
		...
		$(".patient_found").load("patient_list_mediplan.php?key=" + $("#insurance").val() + "&day=" + $("#day").val() + "&month=" + $("#month").val() + "&year=" + $("#year").val() + "&post=" + $("#post").val());
	});
