...
if(strlen($_GET['key'])){
	...
	$patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT pa_records.DocID, pa_records.PatientRecordID, pa_records.FamilyCategory, pa_records.DateIn, pa_records.InsuranceNameID, COALESCE(pa_records.InsuranceCardID, pa_records.applicationNumber, '' ) AS InsuranceCardID, pa_info.*, ad_village.*, ad_cell.*, ad_sector.*, ad_district.* FROM pa_records, pa_info, ad_village, ad_cell, ad_sector, ad_district, sy_users, sy_center WHERE pa_info.PatientID = pa_records.PatientID && pa_info.VillageID=ad_village.ViillageID && ad_village.CellID=ad_cell.CellID && ad_cell.SectorID=ad_sector.SectorID && ad_sector.DistrictID=ad_district.DistrictID && sy_users.CenterID = sy_center.CenterID && {$sys}  && InsuranceNameID='{$_GET['key']}' && DateIn LIKE('{$_GET['year']}-{$_GET['month']}-{$_GET['day']}') {$sp_condition} ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>ID</th><th>Code Beneficiaire</th><th>Name</th><th>Chef de Famille</th><th>Consul.</th><th>Labo</th><th>Price</th><th colspan=4>Medicines</th><th>Hosp</th><th>Price</th><th>Actes</th><th>Price</th><th>Consum.</th><th>Price</th><th>G. Total</th><th>TM</th><th>Payable Total</th></tr>
		...
		for($i=0;$i<count($patients);$i++){
			...
			$kk_ .= "<tr>
					...
					<a target='_blank' href='../forms/".returnSingleField($sql="SELECT FormFile from in_forms WHERE InsuranceNameID='{$r['InsuranceNameID']}'",$field="FormFile",true, $con)."?records={$r['PatientRecordID']}' style='color:blue; text-decoration:none;'>{$total}</a>
					</td>
			</tr>";
			/* if($tot > 0){
				$kk .= $kk_;
