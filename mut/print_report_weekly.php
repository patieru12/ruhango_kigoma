...
if($_GET['format'] == "excel" && @$_SESSION['report']){
	...
	//echo $file_name;
		...
		for($k=0;$k<count($data);$k++){
			...
			foreach($data[$k] as $v){
				...
				if($v === "NUMBER OF PATIENTS FREQUENTING HEALTH FACILITIES"){
					//echo $v; echo $row;echo "<br />";
					SpanCells($activeSheet,"C".$row.":H".($row),$align='center');
					$activeSheet->setCellValue("C".$row, $v);
					$first_column = 'I';
				} 
				if($v === "SPECIAL CASES TO BE REPORTED"){
					//echo $v; echo $row;echo "<br />";
					SpanCells($activeSheet,"I".$row.":M".($row),$align='center');
					$activeSheet->setCellValue("I".$row, $v);
					$first_column = 'N';
				} 
				if($v === "ESTIMATED COST OF MEDICAL BENEFITS TO CBHI MEMBERS"){
					//echo $v; echo $row;echo "<br />";
					SpanCells($activeSheet,"N".$row.":S".($row),$align='center');
					$activeSheet->setCellValue("N".$row, $v);
					//$first_column = 'N';
				} 
				if($v === "Out Patient"){
					//echo $v; echo $row;echo "<br />";
					SpanCells($activeSheet,"C".$row.":E".($row),$align='center');
					$activeSheet->setCellValue("C".$row, $v);
					$first_column = 'F';
				}
				...
				if($v === "In Patient"){
					//echo $v; echo $row;echo "<br />";
					SpanCells($activeSheet,"F".$row.":H".($row),$align='center');
					$activeSheet->setCellValue("F".$row, $v);
					$first_column = 'I';
				}
				
				if($v === "Indigent"){
					//echo $v; echo $row;echo "<br />";
					SpanCells($activeSheet,"I".$row.":I".($row + 1),$align='center');
					$activeSheet->setCellValue("I".$row, $v);
					//$first_column = 'I';
					$auto = 1;
				} else if($k == 1 && $auto == 1){
		...
		$objPHPExcel->getActiveSheet()->getStyle("A".($start_row).":S".(--$row))->applyFromArray($styleArray);
