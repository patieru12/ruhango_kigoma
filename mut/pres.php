...
?>
  ...
  <div id="w">
    <div id="content">
      <h1 style='margin-top:-55px'>RSSB CBHI PRESTATION DOCUMENT</h1>
      <b>
...
<table class="ds_box" cellpadding="0" cellspacing="0" id="ds_conclass"
	...
	<tr>
	  ...
	  <table class=list-1><tr><td>Post</td><td>Start Date</td><td>End Date</td><td>Day</td></tr>
	  <tr>
	  <td>
	  <?php
		...
		?>
		<td>
	  <input type=text name=start_date class=txtfield1 onblur='setTimeout("ds_hi()",3000)' style='width:100px;' id=start_date onclick='ds_sh(this,"start_date")' value='<?= date("Y-m-d",time()) ?>' />
	  </td><td>
	  <input type=text  name=end_date class=txtfield1 onblur='setTimeout("ds_hi()",3000)' style='width:100px;' id=end_date onclick='ds_sh(this,"end_date")' value='<?= date("Y-m-d",time()) ?>' />
	  </td>
	  <td>
	 ...
	 <div id="ds" style='min-width:100px'></div></td></tr></table>
	  ...
	  <div class="patient_found" style='max-height:430px;'>
		<span class=error-text>Select Post to view CBHI Weekly Report</span>
	  </div>
...
$(document).ready(function(){
	...
	$("#generate").click(function(e){
		...
		$(".patient_found").load("prest_data.php?key=" + $("#insurance").val() + "&start_date=" + $("#start_date").val() + "&end_date=" + $("#end_date").val() + "&post=" + $("#post").val() + "&filter=" + $("#filter_").val());
	});
