...
$service = returnAllData("SELECT * FROM se_name WHERE ServiceNameID='".PDB($_GET['service'],true,$con)."'",$con);
//var_dump($service);
	//var_dump($_POST); //die;
	$error = "";
	...
	if (isset($_POST['save'])) {
		...
		//var_dump($name); die;
		if(returnSingleField($sql="SELECT ServiceConsultationID FROM se_consultation WHERE ServiceID='{$service[0]['ServiceNameID']}' && ConsulationID='{$cst_id}' && Status=1",$field="ServiceConsultationID",$data=true, $con)){
			$error = "<span class=error-text>The Consultation already assigned to the service!</span>";
		} else{
			//save new data
			if(saveData($sql="INSERT INTO se_consultation SET  ServiceID='{$service[0]['ServiceNameID']}', ConsulationID='{$cst_id}', Date='{$date}', Status=1",$con)){
	...
	<div id='w' style='height: auto;'>
		...
		<tr>
			...
			<div id="content" style='height: auto;'>
				<h1 style='margin-top:-55px'>Available Consultation Services for <?= $service[0]['ServiceName'] ?></h1>
				<b>
					<?= $error ?>
					<?php
	  ...
	  <form action="./consultion_services.php?service=<?= $_GET['service'] ?>" method=post />
	  <?= @$_GET['service']?"<input type=hidden name=service_id value='{$_GET['service']}' />":"" ?>
	  <table class=frm>
		<tr>
			<td>Consultation</td>
			<td>Date</td>
	  ...
	  Service:<?= $service[0]['ServiceName'] ?><br />
	  Code:<?= $service[0]['ServiceCode'] ?><br />
	  Post: <?= @$service['CenterName'] ?>
	  </span>
	  <?php
		$office = returnAllData("SELECT se_consultation.*, co_category.* FROM se_consultation, co_category WHERE se_consultation.ServiceID='{$service[0]['ServiceNameID']}' && se_consultation.ConsulationID = co_category.ConsultationCategoryID && se_consultation.Status=1",$con);
		if($office){
			//var_dump($office);
			...
			foreach($office as $of){
				...
				echo "<td><a style='color:blue;' href='./consultion_services.php?delete=delete&user={$of['ServiceConsultationID']}' onclick='return confirm(\"Delete {$of['ConsultationCategoryName']} From Possible Consultation Service List in {$service[0]['ServiceName']}!\")' />Delete</a></td>";
				echo "</tr>";
			}
			echo "</table>";
		}
