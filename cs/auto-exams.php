...
?>
  <div id="w" style='height: 86%;'>
    <div id="content" style='height: 100%;'>
    	<b>
		<h1 style='align: center; margin-top:-55px;'>Configure automatic prescription according to Exam Result</h1>
    	...
    	<table cellpadding="0" border=0 cellspacing="0" style='font-size:14px; width:100%; height:100%;'>
    		<tr>
    			...
    			<td>
					...
					//var_dump($update);
					?>
					<form action='auto-exams.php' method='post' style='border:0px solid #000;'>
						...
						<table border=0 style='font-size:16px; width:100%; height:230px;'>
			    			...
			    			<tr style='vertical-align:top;'>
								...
								<td colspan=3>
								<?= $error; ?>
								<div style='background-color:#4f4a41; color:#eee; font-weight:bold; padding:2px; border-top-left-radius:5px; border-top-right-radius:5px; margin-top:10px; text-align:center'>Existing Auto Exam Result Configuration</div>
								<div class=auto_presc_data style='border:1px solid #4f4a41; padding-bottom:5px; border-bottom-left-radius:5px; border-bottom-right-radius:5px; height:390px; padding:2px;'>
									<?php
									//select all saved data 
									$auto_diag = returnAllData($sql="SELECT DISTINCT ResultID, Type, PrescriptionID FROM auto_exams ORDER BY Date DESC ",$con);
									if($auto_diag){
										?>
										<table style="font-size:13px; width:100%" border=1>
											<tr><th>#</th><th>Exam Result</th><th>Type</th><th>Prescription</th><th>Quantity</th><th>Date</th><th colspan=2>Action</th></tr>
											<?php
											//var_dump($auto_diag);
											for($i=0;$i<count($auto_diag);$i++){
												echo "<tr>";
													echo "<td>".($i + 1)."</td>";
													$data_ = returnSingleField("SELECT ResultName FROM la_result WHERE ResultID='{$auto_diag[$i]['ResultID']}'","ResultName",true,$con);
													$ommitted = false;
													...
													$qty_ = returnSingleField("SELECT Quantity FROM `auto_exams` WHERE `ResultID`='{$auto_diag[$i]['ResultID']}' && `PrescriptionID`='{$auto_diag[$i]['PrescriptionID']}' ORDER BY Date DESC LIMIT 0, 1","Quantity",true,$con);
													echo "<td>".$qty_."</td>";
													
													$date_ = returnSingleField("SELECT Date as Quantity FROM `auto_exams` WHERE `ResultID`='{$auto_diag[$i]['ResultID']}' && `PrescriptionID`='{$auto_diag[$i]['PrescriptionID']}' ORDER BY Date DESC LIMIT 0, 1","Quantity",true,$con);
													echo "<td>".$date_."</td>";
													$id = returnSingleField("SELECT AutoActID FROM `auto_exams` WHERE `ResultID`='{$auto_diag[$i]['ResultID']}' && `PrescriptionID`='{$auto_diag[$i]['PrescriptionID']}' ORDER BY Date DESC LIMIT 0, 1","AutoActID",true,$con);
													?>
													<td><a href='./auto-exams.php?update=<?= $id ?>' style='color:blue; text-decoration:none;' title="Update <?= $data_ ?>">Update</a></td>
													<td><a href='./auto-exams.php?delete=<?= $id ?>' style='color:blue; text-decoration:none;' title="Delete <?= $data_ ?>" onclick='return confirm("Delete <?= $data ?> \nFrom Automatic Prescription List ")'>Delete</a></td>
													<?php
...
<script>
	...
	$(".diagnostics").load("./auto/la.php?type=master<?= @$update['ResultID']?"&exist=".$update['ResultID']:"" ?>");
	$(".auto_presc_data").load("./auto/preview.php?type=la");
