...
for($id=0;$id < $objPHPExcel->getSheetCount();$id++){
	...
	for ($row = 1; $row <= $highestRow; $row++){ 
		...
		if(!is_float($rowData[0][0]) && trim($rowData[0][0])){
			...
			if(!$category_id = returnSingleField($sql = "SELECT DiagnosticCategoryID FROM co_diagnostic_category WHERE CategoryCode='".PDB($rowData[0][0],true,$con)."'",$field="DiagnosticCategoryID",$data=true, $con)){
				$category_id = saveAndReturnID($sql = "INSERT INTO co_diagnostic_category SET CategoryName='".PDB($rowData[0][1],true,$con)."', CategoryCode='".PDB($rowData[0][0],true,$con)."'", $con);
			}
			/* 
			$a = 3;
			if($a== 0){ // 0 if(false)
