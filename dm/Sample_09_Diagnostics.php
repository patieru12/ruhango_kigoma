...
if(count($posts)>1){
	...
	echo "<span class=error>Please Select the Post to View Some Information</span>";
	die;
}
...
require_once "../plugins/sis_v1.1.php";
...
$styleCellRight = array('valign' => 'center',"alignment"=> \PhpOffice\PhpWord\SimpleType\Jc::END);
...
//var_dump($_POST);
$tableContent = "<table class=list style='width:100%;'>";
	$tableContent .= "<thead>";
		$tableContent .= "<tr>";
		$table->addRow(30);
			$secondRow = "";
			$thirdRow = "";
			$tableContent .= "<th rowspan=3 style='width:30%;'>Diagnostic</th>";

			// Add the Dignostic Cell
			$cell1 = $table->addCell($table_width*0.16, $cellRowSpan);
			$textrun1 = $cell1->addTextRun($cellHCentered);
			$textrun1->addText(htmlspecialchars('Diagnostic', ENT_COMPAT, 'UTF-8'));
			// $textrun1->addFootnote()->addText(htmlspecialchars('Row span', ENT_COMPAT, 'UTF-8'));


			foreach ($ReportAgeRange  as $key) {
				$tableContent .= "<th colspan=4>".$key."</th>";
				$thirdRow .= "<th colspan=2>Male</th><th colspan=2>Female</th>";
				$secondRow .= "<th>NC</th><th>AC</th><th>NC</th><th>AC</th>";

				$cell2 = $table->addCell($table_width*0.12, $cellColSpan4);
				$textrun2 = $cell2->addTextRun($cellHCentered);
				$textrun2->addText(htmlspecialchars($key, ENT_COMPAT, 'UTF-8'));
			}

			$tableContent .= "<th colspan=4>Total</th>";

			$cell2 = $table->addCell($table_width*0.12, $cellColSpan4 );
			$textrun2 = $cell2->addTextRun($cellHCentered);
			$textrun2->addText(htmlspecialchars('Total', ENT_COMPAT, 'UTF-8'));
		...
		$tableContent .= "<tr>";
			$tableContent .= $thirdRow;

			$cell2 = $table->addCell($table_width*0.064, $cellColSpan2);
			$textrun2 = $cell2->addTextRun($cellHCentered);
			$textrun2->addText(htmlspecialchars("Male", ENT_COMPAT, 'UTF-8'));
			$cell2 = $table->addCell($table_width*0.064, $cellColSpan2);
			$textrun2 = $cell2->addTextRun($cellHCentered);
			$textrun2->addText(htmlspecialchars("Female", ENT_COMPAT, 'UTF-8'));
		...
		$table->addCell($table_width*0.0321, $cellVCentered)->addText(htmlspecialchars('NC', ENT_COMPAT, 'UTF-8'), null, $cellHCentered);
		$table->addCell($table_width*0.0321, $cellVCentered)->addText(htmlspecialchars('AC', ENT_COMPAT, 'UTF-8'), null, $cellHCentered);
		$table->addCell($table_width*0.0321, $cellVCentered)->addText(htmlspecialchars('NC', ENT_COMPAT, 'UTF-8'), null, $cellHCentered);
		$table->addCell($table_width*0.0321, $cellVCentered)->addText(htmlspecialchars('AC', ENT_COMPAT, 'UTF-8'), null, $cellHCentered);
		...
		$table->addCell($table_width*0.0321, $cellVCentered)->addText(htmlspecialchars('AC', ENT_COMPAT, 'UTF-8'), null, $cellHCentered);
		$tableContent .= "<tr>";
			$tableContent .= $secondRow;
		$tableContent .= "</tr>";
	$tableContent .= "</thead>";
	$tableContent .= "<tbody>";
	/* Here Make the use some tools to identify every thing */
	$resultSet = array();
	$minDateIn = PDB($_POST['start_date'], true, $con);
	$maxDateIn = PDB($_POST['end_date'], true, $con);
	if($minDateIn > $maxDateIn){
		$tempDate = $minDateIn;
		$minDateIn = $maxDateIn;
		$maxDateIn = $tempDate;
	}
	$diagnosticNames = array();
	getDiagnosticUsed($diagnosticNames, $minDateIn, $maxDateIn, $center_condition, $con);
	// var_dump($diagnosticNames); 
	// echo "<hr />";
	getDiagnosticRecords($resultSet, $minDateIn, $maxDateIn, $center_condition, $con);
	$returnData = array();
	getOrganizedData($diagnosticNames, $resultSet, $returnData, $diagnosticReportAgeRange);
	// die();
	foreach($diagnosticNames AS $diagnostic){
		// var_dump($diagnostic); die();
		$table->addRow(30);
		$tableContent .= "<tr>";
			$tableContent .= "<td>".$diagnostic['DiagnosticName']."</td>";
			// $table->addCell($table_width*0.0321, $cellVCentered)->addText(htmlspecialchars('AC', ENT_COMPAT, 'UTF-8'), null, $cellHCentered);
			$table->addCell($table_width*0.0321)->addText(htmlspecialchars($diagnostic['DiagnosticName'], ENT_COMPAT, 'UTF-8'));
			$totalMaleNC = 0;
			$totalMaleAC = 0;
			$totalFemaleNC = 0;
			$totalFemaleAC = 0;
			foreach ($ReportAgeRange  as $key=>$v) {
				$value = $diagnosticReportAgeRange[$key];
				// $tableContent .= "<td>".."</td>";
				$tableContent .= "<td class='number'>".$returnData[$diagnostic['DiagnosticID']][$key]["Male"]["NC"]."</td>";
				$table->addCell($table_width*0.0321)->addText(htmlspecialchars($returnData[$diagnostic['DiagnosticID']][$key]["Male"]["NC"], ENT_COMPAT, 'UTF-8'));
			
				$tableContent .= "<td class='number'>".$returnData[$diagnostic['DiagnosticID']][$key]["Male"]["AC"]."</td>";
				$table->addCell($table_width*0.0321)->addText(htmlspecialchars($returnData[$diagnostic['DiagnosticID']][$key]["Male"]["AC"], ENT_COMPAT, 'UTF-8'));

				$tableContent .= "<td class='number'>".$returnData[$diagnostic['DiagnosticID']][$key]["Female"]["NC"]."</td>";
				$table->addCell($table_width*0.0321)->addText(htmlspecialchars($returnData[$diagnostic['DiagnosticID']][$key]["Female"]["NC"], ENT_COMPAT, 'UTF-8'));

				$tableContent .= "<td class='number'>".$returnData[$diagnostic['DiagnosticID']][$key]["Female"]["AC"]."</td>";
				$table->addCell($table_width*0.0321)->addText(htmlspecialchars($returnData[$diagnostic['DiagnosticID']][$key]["Female"]["AC"], ENT_COMPAT, 'UTF-8'));

				$totalMaleNC 	+= $returnData[$diagnostic['DiagnosticID']][$key]["Male"]["NC"];
				$totalMaleAC 	+= $returnData[$diagnostic['DiagnosticID']][$key]["Male"]["AC"];
				$totalFemaleNC 	+= $returnData[$diagnostic['DiagnosticID']][$key]["Female"]["NC"];
				$totalFemaleAC 	+= $returnData[$diagnostic['DiagnosticID']][$key]["Female"]["AC"];
			}
			$tableContent .= "<td class='number totals'>{$totalMaleNC}</td>";
			$table->addCell($table_width*0.0321)->addText(htmlspecialchars($totalMaleNC, ENT_COMPAT, 'UTF-8'));

			$tableContent .= "<td class='number totals'>{$totalMaleAC}</td>";
			$table->addCell($table_width*0.0321)->addText(htmlspecialchars($totalMaleAC, ENT_COMPAT, 'UTF-8'));

			$tableContent .= "<td class='number totals'>{$totalFemaleNC}</td>";
			$table->addCell($table_width*0.0321)->addText(htmlspecialchars($totalFemaleNC, ENT_COMPAT, 'UTF-8'));

			$tableContent .= "<td class='number totals'>{$totalFemaleAC}</td>";
			$table->addCell($table_width*0.0321)->addText(htmlspecialchars($totalFemaleAC, ENT_COMPAT, 'UTF-8'));
...
$objWriter->save('./results/test2.docx');
?>
<div id="reportContent" style="height: 450px; overflow: auto; border:0px solid #000;" >
	<?php
	echo $tableContent;
	?>
</div>
<a href="./results/test2.docx" target="_blank" id="download" name="download" value="" class="flatbtn-blu"> Download</a>
<span class="print_result"></span>
<script type="text/javascript">
	/*$("#download").click(function(e){
		var download = $("#reportContent").html();
		// $(".print_result").html('<img style="height:25px;" src="../images/loading.gif" alt="Generating Word Format. Please Wait"/>'); 
		$.ajax({
			type: "POST",
			url: "./printWordFormat.php",
			data: "data=" + download + "&url=ajax",
			cache: false,
			target: blank,
